...
static int roster_purge(XdbSqlDatas * self, const char *user)
{
	...
	if (!query1) {		/* no query - eep! */
		log_error(NULL,
			  "--!!-- WTF? roster-purge-1 query not found?");
		return 0;
	...
	if (!query2) {		/* no query - eep! */
		log_error(NULL,
			  "--!!-- WTF? roster-purge-2 query not found?");
		return 0;
 ...
 /***** Pass 1: Purge roster elements *****/
	...
	if (!result) {		/* the query failed - bail out! */
		log_error(NULL, "[roster_purge] query<1> failed : %s",
			  sqldb_error(self));
		return 0;
	...
	xdbsql_querydef_free(qd);
	if (!result) {		/* the query failed - bail out! */
		log_error(NULL, "[roster_purge] query<2> failed : %s",
			  sqldb_error(self));
		return 0;
...
xmlnode xdbsql_roster_get(XdbSqlDatas * self, const char *user)
{
	...
	int ndx_jid, ndx_nickname, ndx_subscription, ndx_ask, ndx_type,
	    ndx_ext, ndx_group;
	int ndx_subscribe;
	...
	if (!query1) {		/* no query - eep! */
		log_error(NULL,
			  "--!!-- WTF? roster-load-1 query not found?");
		return NULL;
	...
	if (!query2) {		/* no query - eep! */
		log_error(NULL,
			  "--!!-- WTF? roster-load-2 query not found?");
		return NULL;
 ...
 /***** Pass 1: Load roster elements *****/
	...
	xdbsql_querydef_free(qd);
	if (!result) {		/* the query failed - bail out! */
		log_error(NULL, "[xdbsql_roster_get] query failed : %s",
			  sqldb_error(self));
		return NULL;
	...
	if (!sqldb_use_result(result)) {	/* unable to retrieve the result */
		log_error(NULL,
			  "[xdbsql_roster_get] result fetch failed : %s",
			  sqldb_error(self));
		return NULL;
	...
	while (sqldb_next_tuple(result) != 0) {	/* look for all roster entries and add them to our output tree */
		if (first) {	/* initialize the column mapping indexes */
			...
			ndx_nickname =
			    xdbsql_colmap_index(map, "nickname");
			ndx_subscription =
			    xdbsql_colmap_index(map, "subscription");
			ndx_ask = xdbsql_colmap_index(map, "ask");
			ndx_subscribe =
			    xdbsql_colmap_index(map, "subscribe");
		...
		if (!tmp_str) {
			log_error(NULL,
				  "[xdbsql_roster_get] error getting 'extension' field\n");
			return NULL;
		}
		if (tmp_str && *tmp_str) {
			xmlnode node;
			...
			spooler(s, "<", tmp_attr, ">", tmp_str, "</",
				tmp_attr, ">", s);
			tmp_str = spool_print(s);
			node =
			    xmlnode_str((char *) tmp_str, strlen(tmp_str));
		...
		xmlnode_put_attrib(x1, "jid",
				   sqldb_get_value(result, ndx_jid));
		...
		if (!tmp_str) {
			log_error(NULL,
				  "[xdbsql_roster_get] error getting 'subscription' field\n");
			return NULL;
		}
		...
		if (!tmp_str) {
			log_error(NULL,
				  "[xdbsql_roster_get] error getting 'ask' field\n");
			return NULL;
		}
		...
		if (!tmp_str) {
			log_error(NULL,
				  "[xdbsql_roster_get] error getting 'subscribe' field\n");
			return NULL;
		}
...
static int test_root_roster_node(xmlnode root)
{
	...
	if (j_strcmp(xmlnode_get_attrib(root, "xmlns"), NS_ROSTER) != 0) {	/* it's not got the right namespace */
		log_error(NULL,
			  "[xdbsql_roster_set] not a jabber:iq:roster node");
		return 0;
...
int xdbsql_roster_set(XdbSqlDatas * self, const char *user, xmlnode roster)
{
	...
	if (!query1) {		/* no query - eep! */
		log_error(NULL,
			  "--!!-- WTF? roster-add-1 query not found?");
		return 0;
	...
	if (!query2) {		/* no query - eep! */
		log_error(NULL,
			  "--!!-- WTF? roster-add-2 query not found?");
		return 0;
	...
	for (x1 = xmlnode_get_firstchild(roster); x1; x1 = xmlnode_get_nextsibling(x1)) {	/* create the query definition and begin binding variables */
		...
		if (j_strcmp(tmp_attr, "none") == 0)
			...
			tmp_var[0] = 'B';
		if (tmp_var[0])
			xdbsql_querydef_setvar(qd, "subscription",
					       tmp_var);
		...
		if (!result) {	/* the query failed - bail out! */
			log_warn(ZONE,
				 "[xdbsql_roster_set] query failure on header"
				 " for %s/%s : %s", user,
				 xmlnode_get_attrib(x1, "jid"),
				 sqldb_error(self));
			continue;
		...
		for (x2 = xmlnode_get_firstchild(x1); x2; x2 = xmlnode_get_nextsibling(x2)) {	/* create query to add group and bind its variables */
			...
			result =
			    sqldb_query(self,
					xdbsql_querydef_finalize(qd));
			xdbsql_querydef_free(qd);
			if (!result) {	/* the query failed - bail out! */
				log_warn(ZONE,
					 "[xdbsql_roster_set] query failure on groups"
					 " for %s/%s, halting group add : %s",
					 user, tmp_attr,
					 sqldb_error(self));
				break;
