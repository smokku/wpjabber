...
xmlnode xdbsql_filter_get(XdbSqlDatas *self, const char *user){
	...
	log_error(NULL,"[xdbsql_filter_get] result fetch failed : %s",
		  sqldb_error(self));
	return NULL;
    ...
    */
	...
	if (first){ /* figure out all the indexes for our columns */
	    ...
	    ndx_unavailable = xdbsql_colmap_index(map,"unavailable");
	    ...
	    ndx_continue    = xdbsql_colmap_index(map,"continue");
	...
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"from"),sptr,-1);

	sptr = sqldb_get_value(result, ndx_resource   );
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"resource"),sptr,-1);

	sptr = sqldb_get_value(result, ndx_subject    );
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"subject"),sptr,-1);

	sptr = sqldb_get_value(result, ndx_body       );
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"body"),sptr,-1);

	sptr = sqldb_get_value(result, ndx_show       );
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"show"),sptr,-1);

	sptr = sqldb_get_value(result, ndx_type       );
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"type"),sptr,-1);
	...
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"forward"),sptr,-1);

	sptr = sqldb_get_value(result, ndx_reply      );
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"reply"),sptr,-1);
	...
	if (sptr && *sptr)
	  xmlnode_insert_cdata(xmlnode_insert_tag(rule,"settype"),sptr,-1);
...
static int purge_filter(XdbSqlDatas *self,
			const char *user){
    xmlnode query;	       /* the query for this function */
    query_def qd;	       /* the query definition */
    ...
    if (!query){ /* no query - eep! */
	log_error(NULL,"--!!-- WTF? filter-remove query not found?");
	return 0;
...
static int test_root_filter_node(
    xmlnode root){
	...
	log_error(NULL,"[xdbsql_filter_set] not a jabber:iq:filter node");
	return 0;
...
int xdbsql_filter_set(XdbSqlDatas *self, const char *user, xmlnode data){
	...
	for (x2=xmlnode_get_firstchild(x); x2; x2=xmlnode_get_nextsibling(x2)){ /* handle all the child nodes of the message */
	    ...
	    if (j_strcmp(xmlnode_get_name(x2),"unavailable")==0)
	      data_unavailable = "-";
	    else if (j_strcmp(xmlnode_get_name(x2),"from")==0)
	      data_from        = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"resource")==0)
	      data_resource    = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"subject")==0)
	      data_subject     = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"body")==0)
	      data_body        = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"show")==0)
	      data_show        = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"type")==0)
	      data_type        = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"offline")==0)
	      data_offline     = "-";
	    else if (j_strcmp(xmlnode_get_name(x2),"forward")==0)
	      data_forward     = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"reply")==0)
	      data_reply       = xmlnode_get_data(xmlnode_get_firstchild(x2));
	    else if (j_strcmp(xmlnode_get_name(x2),"continue")==0)
	      data_continue    = "-";
	    else if (j_strcmp(xmlnode_get_name(x2),"settype")==0)
	      data_settype     = xmlnode_get_data(xmlnode_get_firstchild(x2));
	} /* end for */

	/* Fill in the rest of the query parameters. */
	if (data_unavailable && *data_unavailable)
	  xdbsql_querydef_setvar(qd,"unavailable",data_unavailable);
	if (data_from	     && *data_from	 )
	  xdbsql_querydef_setvar(qd,"from"	 ,data_from	  );
	if (data_resource    && *data_resource	 )
	  xdbsql_querydef_setvar(qd,"resource"	 ,data_resource   );
	...
	if (data_offline     && *data_offline	 )
	  xdbsql_querydef_setvar(qd,"offline"	 ,data_offline	  );
	if (data_forward     && *data_forward	 )
	  xdbsql_querydef_setvar(qd,"forward"	 ,data_forward	  );
	if (data_reply	     && *data_reply	 )
	  xdbsql_querydef_setvar(qd,"reply"	 ,data_reply	  );
	if (data_continue    && *data_continue	 )
	  xdbsql_querydef_setvar(qd,"continue"	 ,data_continue   );
	if (data_settype     && *data_settype	 )
	  xdbsql_querydef_setvar(qd,"settype"	 ,data_settype	  );
	...
	xdbsql_querydef_free(qd);
	if (!result)
	    log_error(NULL,"[xdbsql_filter_set] query failed : %s",
		      sqldb_error(self));
	else
	    sqldb_free_result(result);
    } /* end for */
