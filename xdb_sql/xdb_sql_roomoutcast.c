...
xmlnode
xdbsql_roomoutcast_get(XdbSqlDatas *self, const char *room){
	...
	log_error(NULL,"[xdbsql_roomoutcast_get] room not specified");
	return NULL;
    }
    ...
    if (!query){
	log_error(NULL,"--!!-- WTF? roomoutcast-get query not found?");
	return NULL;
    ...
    if (!result){
	log_error(NULL,"[xdbsql_roomoutcast_get] query failed : %s", sqldb_error(self));
	return NULL;
    }

    /* get the results from the query */
    if (!sqldb_use_result(result)){
	log_error(NULL,"[xdbsql_roomoutcast_get] result fetch failed : %s", sqldb_error(self));
	return NULL;
    }
	...
	if (first){
	    ...
	    ndx_responsibleid = xdbsql_colmap_index(map,"responsibleid");
	    ndx_responsiblenick = xdbsql_colmap_index(map,"responsiblenick");
	...
	xmlnode_insert_cdata(reason,
	    sqldb_get_value(result, ndx_reason), -1);
	xmlnode_put_attrib(reason, "actor",
	    sqldb_get_value(result, ndx_responsibleid));
	xmlnode_put_attrib(reason, "nick",
	    sqldb_get_value(result, ndx_responsiblenick));
...
static int
roomoutcast_purge(XdbSqlDatas *self, const char *room){
    ...
    if (!query){
	log_error(NULL,"--!!-- WTF? roomoutcast-remove query not found?");
	return 0;
    }
...
xdbsql_roomoutcast_set(XdbSqlDatas *self, const char *room, xmlnode data){
    ...
    if (!room){
	/* the room was not specified - we have to bug off */
	log_error(NULL,"[xdbsql_roomoutcast_set] room not specified");
	return 0;
    }
    ...
    if (!query){
	log_error(NULL,"--!!-- WTF? roomoutcast-set query not found?");
	return 0;
    }
	...
	data_userid = data_reason = data_responsibleid = data_responsiblenick = NULL;
	...
	if (j_strcmp(name,"item")==0){
		...
		if (j_strcmp(name,"reason")==0){
		    data_responsibleid = xmlnode_get_attrib(x2, "actor");
		    data_responsiblenick = xmlnode_get_attrib(x2, "nick");
	...
	if (data_responsibleid && *data_responsibleid)
	  xdbsql_querydef_setvar(qd, "responsibleid", data_responsibleid);
	if (data_responsiblenick && *data_responsiblenick)
	  xdbsql_querydef_setvar(qd, "responsiblenick", data_responsiblenick);
	...
	if (!result)
	    log_error(NULL,"[xdbsql_roomoutcast_set] query failed : %s",
		sqldb_error(self));
	else
	    sqldb_free_result(result);
    }
