...
static XdbODBCResult *xdbodbc_query(XdbODBCBackend * self,
				    const char *query);
...
static void store_odbc_error(SQLSMALLINT htype, SQLHANDLE hndl,
 ...
 */
int xdbodbc_connect(XdbODBCBackend * self, const char *host,
		    ...
		    const char *db)
{
	...
	SQLSetEnvAttr(self->env, SQL_ATTR_ODBC_VERSION,
		      (void *) SQL_OV_ODBC3, 0);
...
short xdbodbc_is_connected(XdbODBCBackend * self)
{
	...
	ret =
	    SQLGetInfo(self->dbc, SQL_DATA_SOURCE_NAME, buffer,
...
short xdbodbc_store_result(XdbODBCResult * res)
{
	...
	while (SQL_SUCCEEDED(SQLFetch(res->stmt))) {
		...
		if (res->tuple_count == res->tuple_internal_count) {
			...
			res->tuples = (Tuple *) realloc((void *) res->
							tuples,
							(res->
							 tuple_internal_count
							 +
							 TUPLE_GRANULARITY)
							* sizeof(Tuple));
		...
		/* Init a tuple */
		tuple_alloc_fields(&res->tuples[res->tuple_count],
				   res->field_count);
...
int xdbodbc_escape_string(XdbODBCBackend * self, char *to,
			  const char *from, int len)
{
	unsigned char c;
	int i;
...
static void tuple_alloc_fields(Tuple * tuple, int field_count)
{
	...
	tuple->fields =
	    (SQLCHAR **) malloc(field_count * sizeof(SQLCHAR *));
	if (!tuple->fields) {
		log_error(ZONE,
...
static void tuple_get_data(Tuple * tuple, SQLHSTMT stmt)
{
	...
	for (pos = 0; pos < tuple->field_count; ++pos) {
		...
		tuple->fields[pos] =
		    (SQLCHAR *) malloc((FIELD_INITIAL_SIZE + 1) *
				       sizeof(SQLCHAR));
		...
		LOCAL_DEBUG(printf
			    (" Read %d bytes out of %d\n",
			     FIELD_INITIAL_SIZE, read_len));

		/* Expand field */
		tuple->fields[pos] = (SQLCHAR *) realloc((void *) tuple->
							 fields[pos],
							 (read_len +
							  1) *
							 sizeof(SQLCHAR));
		...
		LOCAL_DEBUG(printf
			    (" Read %d bytes of remaining data\n",
			     read_len));
	}
}
