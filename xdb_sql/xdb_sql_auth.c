...
xmlnode
xdbsql_auth_get(XdbSqlDatas *self, const char *user){
	...
	log_error(NULL,"[xdbsql_auth_get] result fetch failed : %s",
		  sqldb_error(self));
	return NULL;
	...
	if (row){ /* use a column map to get the index of the "password" field */
	    ...
	    log_error(NULL,"[xdbsql_auth_get] row fetch failed");

    } /* end if */
    else{ /* we didn't get back the exact row count we wanted */
	if (rowcount==0)
	    log_error(NULL,"[xdbsql_auth_get] no results, user not found");
	else
	    log_error(NULL,"[xdbsql_auth_get] user not unique");
...
xmlnode
xdbsql_authhash_get(XdbSqlDatas *self, const char *user){
    xmlnode query;	    /* the actual query to be executed */
    xmlnode data = NULL;    /* return data from this function */
    query_def qd;	    /* query definition */
    XdbSqlResult *result;   /* pointer to database result */
    ...
    if (!user){ /* the user was not specified - we have to bug off */
	log_error(NULL,"[xdbsql_authhash_get] user not specified");
	return NULL;
    ...
    if (!query){ /* that query should have been specified... */
	log_error(NULL,"--!!-- WTF? authhash-get query not found?");
	return NULL;
    ...
    /* get the results from the query */
    if (!sqldb_store_result(result)){ /* the result fetch failed - bye now! */
	log_error(NULL,"[xdbsql_authhash_get] result fetch failed : %s",
		  sqldb_error(self));
	return NULL;
	...
	if (row){ /* use a column map to get the index of the "password" field */
	    ...
	    log_error(NULL,"[xdbsql_authhash_get] row fetch failed");

    } /* end if */
    else{ /* we didn't get back the exact row count we wanted */
	if (rowcount==0)
	    log_error(NULL,"[xdbsql_authhash_get] no results, user not found");
	else
	    log_error(NULL,"[xdbsql_authhash_get] user not unique");
...
static char *
extract_passwordhash(xmlnode data){
    char *rc;  /* return from this function */
...
int
xdbsql_auth_set(XdbSqlDatas *self, const char *user, xmlnode data){
...
int
xdbsql_authhash_set(XdbSqlDatas *self, const char *user, xmlnode data){
    const char *password;   /* the password set by the user */
    const char *qname;	    /* query name to use */
    xmlnode query;	    /* the actual query to be executed */
    query_def qd;	    /* query definition */
    ...
    if (!user){
	log_error(NULL,"[xdbsql_authhash_set] user not specified");
	return 0;
...
int
xdbsql_user_exists(XdbSqlDatas *self, const char *user){
	...
	log_error(NULL,"[xdbsql_user_exists] result fetch failed : %s",
		  sqldb_error(self));
	return 0;
...
int
xdbsql_userhash_exists(XdbSqlDatas *self, const char *user){
    xmlnode query;	       /* query to be executed */
    query_def qd;	       /* query definition */
    XdbSqlResult *result;      /* pointer to database result */
#ifdef NUM_TUPLES
    ...
    if (!user){
	log_error(NULL,"[xdbsql_userhash_exists] user not specified");
	return 0;
    ...
    xdbsql_querydef_free(qd);
    if (!result){ /* the query failed - bail out! */
	log_error(NULL,"[xdbsql_userhash_exists] query failed : %s",
		  sqldb_error(self));
	return 0;
    ...
    /* get the results from the query */
    if (!sqldb_store_result(result)){ /* unable to retrieve the result */
	log_error(NULL,"[xdbsql_userhash_exists] result fetch failed : %s",
		  sqldb_error(self));
	return 0;
