 ...
 */
xmlnode xdbsql_simple_get(XdbSqlDatas * self, query_def qd,
			  const char *user)
{
	...
	if (!sqlres) {		/* the query failed - bail out! */
		log_error(NULL, "[xdbsql_simple_get] query failed : %s",
			  sqldb_error(self));
		return NULL;
	}
	...
	if (!sqldb_use_result(sqlres)) {	/* unable to retrieve the result */
		log_error(NULL,
			  "[xdbsql_simple_get] result fetch failed : %s",
			  sqldb_error(self));
		return NULL;
	}
			 ...
			 xmlnode item, xmlnode rc)
{
	...
	for (bcol = xdbsql_querydef_get_firstcol(qd);
		...
		/* process value */
		sptr =
		    sqldb_get_value(sqlres,
				    xdbsql_querydef_col_get_offset(bcol));
		...
		if (xdbsql_querydef_col_get_instype(bcol) == INS_TAG) {
			xmlnode tag = xmlnode_insert_tag(dest,
							 xdbsql_querydef_col_get_name
							 (bcol));
			xmlnode_insert_cdata(tag, sptr, -1);
		} else {
			xmlnode_put_attrib(dest,
					   xdbsql_querydef_col_get_name
					   (bcol), sptr);
		}
	}			/* for bcol */
...
static int simple_purge(XdbSqlDatas * self, query_def qd, const char *user)
{
	query_def pq;
	req_query_def rqd;	/* the query definition */
	XdbSqlResult *sqlres;	/* pointer to database result */
	...
	xdbsql_reqquerydef_free(rqd);
	if (!sqlres) {		/* the query failed - bail out! */
		log_error(NULL, "[simple_purge] query failed : %s",
			  sqldb_error(self));
		return 0;
	}
...
static query_def find_purge_query(XdbSqlDatas * self, query_def qd)
{
	...
	if (!pq)
		log_error(NULL, "[find_purge_query] query '%s' not found",
			  pq_name);
		      ...
		      const char *user, xmlnode datas)
{
	if (!user) {
		log_error(NULL,
			  "[xdbsql_query_handler_v2] user not specified");
		return 0;
	}
...
static int substitute_params(req_query_def rqd, xmlnode x)
{
	...
	for (var = xdbsql_reqquerydef_get_firstvar(rqd);
		...
		if (sptr && *sptr) {
			...
			xdbsql_querydef_setvar((query_def) rqd, name,
					       sptr);
			return 1;
		}
	}
