...
xmlnode xdbsql_roomconfig_get(XdbSqlDatas * self, const char *room)
{
	...
	int ndx_owner, ndx_name, ndx_secret, ndx_private, ndx_leave,
	    ndx_join;
	int ndx_rename, ndx_public, ndx_subjectlock, ndx_maxusers,
	    ndx_moderated;
	int ndx_defaultype, ndx_privmsg, ndx_invitation, ndx_invites,
	    ndx_legacy;
	...
	if (!room) {
		...
		log_error(NULL,
			  "[xdbsql_roomconfig_get] room not specified");
		return NULL;
	}
	...
	if (!query) {
		log_error(NULL,
			  "--!!-- WTF? roomconfig-get query not found?");
		return NULL;
	...
	if (!result) {
		log_error(NULL,
			  "[xdbsql_roomconfig_get] query failed : %s",
			  sqldb_error(self));
		return NULL;
	}
	...
	if (!sqldb_use_result(result)) {
		log_error(NULL,
			  "[xdbsql_roomconfig_get] result fetch failed : %s",
			  sqldb_error(self));
		return NULL;
	}
	...
	while (sqldb_next_tuple(result) != 0) {
		...
		if (first) {
			...
			ndx_persistant =
			    xdbsql_colmap_index(map, "persistant");
			ndx_public = xdbsql_colmap_index(map, "public");
			ndx_subjectlock =
			    xdbsql_colmap_index(map, "subjectlock");
			ndx_maxusers =
			    xdbsql_colmap_index(map, "maxusers");
			ndx_moderated =
			    xdbsql_colmap_index(map, "moderated");
			ndx_defaultype =
			    xdbsql_colmap_index(map, "defaultype");
			ndx_privmsg = xdbsql_colmap_index(map, "privmsg");
			ndx_invitation =
			    xdbsql_colmap_index(map, "invitation");
			...
			ndx_logformat =
			    xdbsql_colmap_index(map, "logformat");
			ndx_logging = xdbsql_colmap_index(map, "logging");
			ndx_description =
			    xdbsql_colmap_index(map, "description");
		...
		xmlnode_put_attrib(xmlnode_insert_tag(node, "owner"),
				   "jid", sqldb_get_value(result,
							  ndx_owner));
		xmlnode_insert_cdata(xmlnode_insert_tag(node, "name"),
				     sqldb_get_value(result, ndx_name),
				     -1);
		...
		xmlnode_insert_cdata(xmlnode_insert_tag(node, "maxusers"),
				     sqldb_get_value(result, ndx_maxusers),
				     -1);
		xmlnode_insert_cdata(xmlnode_insert_tag
				     (node, "persistant"),
				     sqldb_get_value(result,
		...
		xmlnode_insert_cdata(xmlnode_insert_tag(node, "privmsg"),
				     sqldb_get_value(result, ndx_privmsg),
				     -1);
		xmlnode_insert_cdata(xmlnode_insert_tag
				     (node, "invitation"),
				     sqldb_get_value(result,
		...
		xmlnode_insert_cdata(xmlnode_insert_tag(node, "logging"),
				     sqldb_get_value(result, ndx_logging),
				     -1);
	...
	if (tuples != 1) {
		if (tuples == 0)
			log_error(NULL,
				  "[xdbsql_roomconfig_get] room not found");
		if (tuples > 1)
			log_error(NULL,
				  "[xdbsql_roomconfig_get] room not unique");
...
static int roomconfig_purge(XdbSqlDatas * self, const char *room)
{
	...
	if (!query) {
		log_error(NULL,
			  "--!!-- WTF? roomconfig-remove query not found?");
		return 0;
	}
...
xdbsql_roomconfig_set(XdbSqlDatas * self, const char *room, xmlnode data)
{
	...
	if (!room) {
		/* the room was not specified - we have to bug off */
		log_error(NULL,
			  "[xdbsql_roomconfig_set] room not specified");
		return 0;
	}
	...
	if (!query) {
		log_error(NULL,
			  "--!!-- WTF? roomconfig-set query not found?");
		return 0;
	}
	...
	if (data_subjectlock && *data_subjectlock)
		xdbsql_querydef_setvar(qd, "subjectlock",
				       data_subjectlock);
	...
	if (data_description && *data_description)
		xdbsql_querydef_setvar(qd, "description",
				       data_description);
	...
	if (!result)
		log_error(NULL,
			  "[xdbsql_roomconfig_set] query failed : %s",
			  sqldb_error(self));
	else
		sqldb_free_result(result);
