...
xmlnode xdbsql_offline_get(XdbSqlDatas *self, const char *user){
	...
	log_error(NULL,"[xdbsql_offline_get] query failed : %s", sqldb_error(self));
	return NULL;
	...
	log_error(NULL,"[xdbsql_offline_get] result fetch failed : %s", sqldb_error(self));
	return NULL;
	...
	if (first){ /* figure out all the indexes for our columns */
	    ...
	    ndx_priority = xdbsql_colmap_index(map,"priority");
	...
	if (sptr && *sptr)
	    xmlnode_insert_cdata(xmlnode_insert_tag(msg,"priority"),sptr,-1);
	sptr = sqldb_get_value(result, ndx_thread);
	if (sptr && *sptr)
	    xmlnode_insert_cdata(xmlnode_insert_tag(msg,"thread"),sptr,-1);
	sptr = sqldb_get_value(result, ndx_subject);
	if (sptr && *sptr)
	    xmlnode_insert_cdata(xmlnode_insert_tag(msg,"subject"),sptr,-1);
...
static int purge_offline(XdbSqlDatas *self,
			 const char *user){
    xmlnode query;	       /* the query for this function */
    query_def qd;	       /* the query definition */
    ...
    if (!query){ /* no query - eep! */
	log_error(NULL,"--!!-- WTF? spool-remove query not found?");
	return 0;
...
static int load_query(query_def qd, xmlnode x, pool p){
  ...
  char *data_subject, *data_thread, *data_priority, *data_body, *data_date;
  ...
  for (x2 = xmlnode_get_firstchild(x); x2; x2 = xmlnode_get_nextsibling(x2)){ /* handle all the child nodes of the message */
	...
	data_priority = xmlnode_get_data(xmlnode_get_firstchild(x2));
      else if (j_strcmp(xmlnode_get_name(x2),"thread")==0)
	data_thread = xmlnode_get_data(xmlnode_get_firstchild(x2));
...
int xdbsql_offline_set(XdbSqlDatas *self, const char *user, xmlnode xdb_request_node){
    xmlnode x;
    xmlnode data;
    xmlnode query;	       /* the query for this function */
    query_def qd;	       /* the query definition */
	...
	if (j_strcmp("insert", action) != 0){
	    log_error(ZONE, "don't know how to handle action (%s)", action);
	    return 1;
	}
	insert_mode = 1;
    }
    ...
    */
	...
	xdbsql_querydef_free(qd);
	if (!result)
	    log_error(ZONE,"query failed : %s", sqldb_error(self));
	else
	    sqldb_free_result(result);
    }
    else{
