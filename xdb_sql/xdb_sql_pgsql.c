...
static XdbPgsqlResult *xdbpgsql_query(XdbPgsqlBackend * self,
				      const char *query);
 ...
 */
int xdbpgsql_connect(XdbPgsqlBackend * self, const char *host,
...
{
	if (!self->connection
	    || PQstatus(self->connection) != CONNECTION_OK)
		return 0;
	return 1;
}
...
int xdbpgsql_escape_string(XdbPgsqlBackend * self, char *to,
			   const char *from, int len)
{
	unsigned char c;
	int i;
