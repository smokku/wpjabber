...
xmlnode xdbsql_vcard_get(XdbSqlDatas * self, const char *user)
{
	...
	if (!sqldb_use_result(result)) {	/* unable to retrieve the result */
		log_error(NULL,
			  "[xdbsql_vcard_get] result fetch failed : %s",
			  sqldb_error(self));
		return NULL;
	...
	while (sqldb_next_tuple(result)) {	/* look for all vcard vCards and add them to the list */
		if (first) {	/* figure out all the indexes for our columns */
			...
			ndx_full_name =
			    xdbsql_colmap_index(map, "full_name");
			ndx_first_name =
			    xdbsql_colmap_index(map, "first_name");
			ndx_last_name =
			    xdbsql_colmap_index(map, "last_name");
			ndx_nick_name =
			    xdbsql_colmap_index(map, "nick_name");
			ndx_url = xdbsql_colmap_index(map, "url");
			ndx_address1 =
			    xdbsql_colmap_index(map, "address1");
			ndx_address2 =
			    xdbsql_colmap_index(map, "address2");
			ndx_locality =
			    xdbsql_colmap_index(map, "locality");
			...
			ndx_telephone =
			    xdbsql_colmap_index(map, "telephone");
			...
			ndx_photo_t =
			    xdbsql_colmap_index(map, "photo_type");
			ndx_photo_b =
			    xdbsql_colmap_index(map, "photo_bin");
		...
		if (sptr && *sptr)
			xmlnode_insert_cdata(xmlnode_insert_tag(rc, "FN"),
					     sptr, -1);
		...
		if (sptr && *sptr)
			xmlnode_insert_cdata(xmlnode_insert_tag(rc, "URL"),
					     sptr, -1);
		...
		if (sptr && *sptr)
			xmlnode_insert_cdata(xmlnode_insert_tag(x, "CTRY"),
					     sptr, -1);
		/* end address node */
		...
		if (sptr && *sptr) {
			...
			xmlnode_insert_cdata(xmlnode_insert_tag(x, "TYPE"),
					     sptr, -1);
...
static int purge_vcard(XdbSqlDatas * self, const char *user)
{
	xmlnode query;		/* the query for this function */
	query_def qd;		/* the query definition */
	...
	if (!query) {		/* no query - eep! */
		log_error(NULL,
			  "--!!-- WTF? vcard-remove query not found?");
		return 0;
...
static int test_root_vcard_node(xmlnode root)
{
...
int xdbsql_vcard_set(XdbSqlDatas * self, const char *user, xmlnode data)
{
	...
	log_debug("data_b_day: (%s), b_day_8601: (%s)", data_b_day,
		  b_day_8601);
