...
short xdbmysql_use_result (XdbMysqlResult *res){
    res->result = mysql_use_result(result_get_backend(res)->connection);
    if (!res->result)
	return 0;
    return 1;
}
...
short xdbmysql_store_result (XdbMysqlResult *res){
    res->result = mysql_store_result(result_get_backend(res)->connection);
    if (!res->result)
	return 0;
    res->stored = 1;
    return 1;
 ...
 * 2*len+1 chars.
 */
int xdbmysql_escape_string (XdbMysqlBackend *self, char *to, const char *from,
			    int len){
