...
XdbOracleResult *oracle_query (XdbOracleBackend *self, const char *query, int reporterr);
...
static XdbOracleResult*  xdboracle_query (XdbOracleBackend *self, const char *query);
...
XdbSqlBackend *xdboracle_backend_new (void){
    ...
    if (checkerr(back->errhp, OCIInitialize((ub4) OCI_OBJECT, NULL, NULL, NULL, NULL ), 1)){
	free(back);
	return NULL;
    }
    if (checkerr(back->errhp, OCIEnvInit(&back->envhp, (ub4) OCI_DEFAULT,
	(size_t) 0, (dvoid **)0), 1)){
	free(back);
	return NULL;
    }
    if (checkerr(back->errhp, OCIHandleAlloc(back->envhp, (void**)&back->errhp, OCI_HTYPE_ERROR,
	(size_t) 0, (dvoid **) 0), 1)){
...
*/
int xdboracle_connect (XdbOracleBackend *self, const char *host, const char *port,
		       const char *user, const char *pass, const char *db){
    /* Connect */
    if (checkerr(self->errhp, OCILogon(self->envhp, self->errhp,
	&self->svchp, (text*)user, (ub4)strlen(user),
	(text*)pass, (ub4)strlen(pass), (text*)db,
	(ub4)strlen(db)), 1)){
	return 0;
    }
    DebugPrint("xdboracle_connect: Logged to Oracle, db=%s, user=%s, pass=%s", db, user, pass);
    if (!xdboracle_is_connected(self)){
...
XdbOracleResult *oracle_query (XdbOracleBackend *self, const char *query, int reporterr){
    ...
    /* create a statement */
    if (checkerr(self->errhp, OCIHandleAlloc((dvoid *)self->envhp, (dvoid **)&res->stmthp,
	(ub4)OCI_HTYPE_STMT, (size_t)0, (dvoid **)0),reporterr)){
	free(res);
	return NULL;
    }
	...
	(ub4)OCI_NTV_SYNTAX, (ub4)OCI_DEFAULT), reporterr) ){
	free(res);
	return NULL;
    }
	...
	(ub4)OCI_HTYPE_STMT,
	(dvoid *)&stmt_type, (ub4 *)0,
	(ub4)OCI_ATTR_STMT_TYPE, self->errhp), reporterr)){
	...
	(ub4)OCI_DEFAULT|OCI_COMMIT_ON_SUCCESS), reporterr)){
	OCIHandleFree((dvoid *)res->stmthp, (ub4)OCI_HTYPE_STMT);
	free(res);
	return NULL;
    }
	...
	checkerr(self->errhp, OCIAttrGet((dvoid*) parmdp, (ub4) OCI_DTYPE_PARAM,
	...
	checkerr(self->errhp, OCIAttrGet((dvoid*) parmdp, (ub4) OCI_DTYPE_PARAM,
	    (dvoid*) &fieldlen, (ub4 *) 0, (ub4) OCI_ATTR_DATA_SIZE,
	...
	*/
	...
	res->field_size[res->field_count]=(fieldlen > MAX_FIELD_SIZE ? MAX_FIELD_SIZE : fieldlen);

	/* increment counter and get next descriptor, if there is one */
	counter++;
	res->field_count++;
	parm_status = OCIParamGet(res->stmthp, OCI_HTYPE_STMT,
	    self->errhp, (dvoid**)&parmdp, (ub4) counter);
    }
...
*/
XdbOracleResult *xdboracle_query (XdbOracleBackend *self, const char *query){
...
const char *xdboracle_error (XdbOracleBackend *self){
    ub4 errcode;
    OCIErrorGet ((dvoid *) self->errhp, (ub4) 1, (text *) NULL, (sb4 *)&errcode,
	(text*)self->errbuf, (ub4) sizeof(self->errbuf), (ub4) OCI_HTYPE_ERROR);
...
short xdboracle_store_result (XdbOracleResult *res){
	...
	if (size == 0 ) continue; /* Unsupported */

	/* text */
	buf[i] = malloc(size+1);
	bzero(buf[i],size+1);
	if (buf[i]==NULL){
	    for(;i>=0;i--) free(buf[i]);
	    free(res->tuples);
	    res->tuples=NULL;
	    log_error(ZONE,"Out of memory");
	    return 0;
	...
	checkerr(self->errhp, OCIDefineByPos(res->stmthp, &defhp[i], self->errhp,
	    (ub4)i+1,
	    (dvoid *)buf[i],
	    (sb4)size+1, SQLT_STR,
	    (dvoid *)&inds[i], (ub2 *)0, (ub2 *)0,
	    (ub4)OCI_DEFAULT), 1);
	...
	if (i >= res->tuple_allocated){
	    ...
	    res->tuples = (Tuple*) realloc(res->tuples,
	       res->tuple_allocated*sizeof(Tuple));
	    if (res->tuples == NULL){
	...
	if (status != OCI_SUCCESS){
	    if (status != OCI_NO_DATA) checkerr(self->errhp,status,1);
	    has_more_data = 0;
	} else{
	    tuple_alloc_fields(&res->tuples[i], res->field_count);
	    res->tuple_count = i + 1;
	    for(j=0; j<res->field_count; j++){
		size = res->field_size[j];
		res->tuples[i].fields[j] = malloc(size+1);
		if ( res->tuples[i].fields[j] == NULL ) break;
		strncpy(res->tuples[i].fields[j], buf[j], size);
	    }
	}
    }
    for(i=0; i<res->field_count; i++) free(buf[i]);

    DebugPrint("xdboracle_store_result: fetched %d rows",res->tuple_count);
...
int xdboracle_escape_string (XdbOracleBackend *self, char *to, const char *from,
			     int len){
    unsigned char c;
    int i;
