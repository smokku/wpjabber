...
query_def xdbsql_querydef_init(XdbSqlDatas *self, xmlnode qd_root){
    ...
    for (tmp=xmlnode_get_firstchild(qd_root); tmp; tmp=xmlnode_get_nextsibling(tmp)){ /* look for <text> and <bindvar> tags among the children */
      if (j_strcmp(xmlnode_get_name(tmp),"text")==0){
	    ...
	    var->length = (xmlnode_get_attrib(tmp,"length") != NULL ? atoi(xmlnode_get_attrib(tmp,"length")):0);
	    var->next = NULL;
    ...
    const char *value)		 /* value to replace variable with - NULL to delete */
{
	...
	if (var->need_escape){ /* need to SQL-encode the replacement text! */
	    esc_buffer = (char *)alloca((repl_len * 2 + 1) * sizeof(char));
	    sqldb_escape_string(qd->datas, esc_buffer,ef_value,repl_len);
	}
	...
	if ( var->length > 0 && strlen(esc_buffer) > var->length) esc_buffer[var->length]='\0'; /* truncate the value */
...
bindvar_new (pool p, const char *name, int is_attrib, const char *replace_text,
	     const char *default_value, int need_escape){
 ...
 */
static int
process_node_param (xmlnode x, xmlnode *ptr){
    xmlnode res;
	...
	log_error(NULL,
		  ...
		  " at most 1 '%s' element is allowed", xmlnode_get_name(x));
	return 0;
    }
    ...
    *ptr = res;
	...
	log_error(NULL, "[xdbsql_simple_querydef_init]"
		  " missing child tag for '%s'", xmlnode_get_name(x));
	return 0;
    }
    return 1;
}

static col_map_entry
process_col (xmlnode x, pool p){
    col_map_entry col;
    const char *offset;
    ...
    col = (col_map_entry)pmalloc(p,sizeof(struct col_map_entry_struct));
    /*
...
query_def xdbsql_querydef_init_v2(XdbSqlDatas *self, pool p, xmlnode qd_root){
	...
	if (j_strcmp(xmlnode_get_name(tmp),"text")==0){
	    ...
	    name = (const char *)xmlnode_get_attrib(tmp,"tag");
	    if (!name){
		name = (const char *)xmlnode_get_attrib(tmp,"attrib");
		is_attr = 1;
	    }
	    if (!name){
		log_error(NULL,
		...
		p,
		name, is_attr,
		(const char *)xmlnode_get_data(xmlnode_get_firstchild(tmp)),
		(const char *)xmlnode_get_attrib(tmp,"default"),
		(is_false(xmlnode_get_attrib(tmp,"escape")) ? 0 : 1));
	...
	else if (j_strcmp(xmlnode_get_name(tmp),"top-result")==0){
	    /* (top-result) */
	    if (!process_node_param(tmp, &qd->resnode))
		return NULL;
	}
	else if (j_strcmp(xmlnode_get_name(tmp),"tuple-node")==0){
	    /* (tuple-node)? */
...
req_query_def xdbsql_reqquerydef_init(const query_def qd, const char *user){
    ...
    /* allocate pool and querydef structure */
    p = pool_new();
    reqqd = (req_query_def)pmalloc(p,sizeof(struct req_query_def_struct));
    reqqd->p = p;
    ...
    if (qd->user_var == NULL){
	log_error(NULL, "xdbsql_reqquerydef_init : qd->user_var == NULL");
