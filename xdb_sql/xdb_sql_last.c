...
static int last_purge(XdbSqlDatas * self, const char *username)
{
...
xmlnode xdbsql_last_get(XdbSqlDatas * self, const char *username)
{
	...
	if (!username) {	/* the username was not specified - we have to
				 * bug off */
		log_error(NULL,
			  "[xdbsql_last_get] username not specified");
		return NULL;
	...
	if (!sqldb_use_result(result)) {	/* unable to retrieve the result */
		log_error(NULL,
			  "[xdbsql_last_get] result fetch failed : %s",
			  sqldb_error(self));
		return NULL;
	...
	while (sqldb_next_tuple(result) != 0) {	/* look for all last
		...
		xmlnode_put_attrib(rc, "last",
				   sqldb_get_value(result, ndx_seconds));
...
static int test_root_last_node(xmlnode root)
{
	...
	if (j_strcmp(xmlnode_get_attrib(root, "xmlns"), NS_LAST) != 0) {	/* it has not got the right namespace */
		log_error(NULL,
			  "[xdbsql_last_set] not a jabber:iq:last node");
		return 0;
...
int xdbsql_last_set(XdbSqlDatas * self, const char *username, xmlnode last)
{
	xmlnode query1;		/* query nodes */
	...
	if (!username) {	/* username not specified */
		log_error(NULL,
			  "[xdbsql_last_set] username not specified");
		return 0;
