...
char* srv_lookup(pool p, const char* service, const char* domain){
     ...
     if (service == NULL){
	  ...
	  if (!hp){
	       log_alert("error", "srv: Unable to resolve: %s", domain);
	       return NULL;
	  }
	  else{
     ...
     if (((_res.options & RES_INIT) == 0) && (res_init() == -1)){
	  log_alert("error", "srv: initialization failed on res_init.");
	  return NULL;
     }
	...
	3.) there are 1 or more answers available */
     if ( (replylen > 0) && (ntohs(rheader->rcode) == NOERROR) && (ntohs(rheader->ancount) > 0) ){
	  ...
	  if (exprc < 0){
	       log_debug("srv: DN expansion failed for Question section.");
	       return NULL;
	  }
	  ...
	  while (rrptr < (reply + replylen)){
	       ...
	       if (exprc < 0){
		    log_debug("srv: Whoa nelly! DN expansion failed for RR.");
		    return NULL;
	       }
	       ...
	       case T_SRV:
		    ...
		    if (exprc < 0){
			 log_debug("srv: DN expansion failed for SRV.");
			 return NULL;
		    }
	  ...
	  while (iternode != NULL){
	       ...
	       /* Otherwise, request the A record */
		    ...
		    log_debug("srv: attempting A record lookup.");
		    /* FIXME: Is this really retrv'ing A record? */
		    hp = gethostbyname(iternode->host);
		    if (!hp){
			 log_alert("error", "srv: Unable to resolve SRV reference to: %s\n", iternode->host);
		    }
		    else{
