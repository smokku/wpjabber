...
char *srv_inet_ntoa(pool p, unsigned char *addrptr)
{
	char result[16];
	result[15] = '\0';
	snprintf(result, 16, "%d.%d.%d.%d", addrptr[0], addrptr[1],
		 addrptr[2], addrptr[3]);
...
char *srv_lookup(pool p, const char *service, const char *domain)
{
	...
	if (service == NULL) {
		log_debug("srv: Standard resolution of %s", domain);
		hp = gethostbyname(domain);
		if (!hp) {
			log_alert("error", "srv: Unable to resolve: %s",
				  domain);
			return NULL;
		} else {
	...
	log_debug("srv: SRV resolution of %s.%s", service, domain);

	/* Setup A record hash table */
	arr_table =
	    ghash_create(11, (KEYHASHFUNC) str_hash_code,
			 (KEYCOMPAREFUNC) j_strcmp);

	/* Initialize lookup system if needed (check global _res structure) */
	if (((_res.options & RES_INIT) == 0) && (res_init() == -1)) {
		log_alert("error",
			  "srv: initialization failed on res_init.");
		return NULL;
	}

	/* Run a SRV query against the specified domain */
	replylen = res_querydomain(service, domain, C_IN,	/* Class */
				   T_SRV,	/* Type */
	...
	if ((replylen > 0) && (ntohs(rheader->rcode) == NOERROR)
		...
		if (exprc < 0) {
			log_debug
			    ("srv: DN expansion failed for Question section.");
			return NULL;
		}
		...
		while (rrptr < (reply + replylen)) {
			...
			exprc =
			    dn_expand(reply, reply + replylen, rrptr, host,
				      sizeof(host));
			if (exprc < 0) {
				log_debug
				    ("srv: Whoa nelly! DN expansion failed for RR.");
				return NULL;
			}
			...
			case T_SRV:
				...
				exprc =
				    dn_expand(reply, reply + replylen,
					      rrptr + 6, host,
					      sizeof(host));
				if (exprc < 0) {
					log_debug
					    ("srv: DN expansion failed for SRV.");
					return NULL;
				}
		...
		while (iternode != NULL) {
			...
			ipaddr =
			    (char *) ghash_get(arr_table, iternode->host);
			if (ipaddr != NULL) {
				spooler(result, ipaddr, ":",
					iternode->port, result);
			}
			/* Otherwise, request the A record */
			else {
				log_debug
				    ("srv: attempting A record lookup.");
				/* FIXME: Is this really retrv'ing A record? */
				hp = gethostbyname(iternode->host);
				if (!hp) {
					log_alert("error",
						  "srv: Unable to resolve SRV reference to: %s\n",
						  iternode->host);
				} else {
					spooler(result,
						srv_inet_ntoa(p,
