...
void *wpdns_thread_main(void *arg){
  ...
  while(1){
	...
	if (l){
	  ...
	  str = NULL;
	  while (iternode != NULL){
		str = srv_lookup(l->packet->p, iternode->service, l->host);
		if (str != NULL){
		  log_debug("Resolved %s(%s): %s\tresend to:%s", l->host, iternode->service, str, iternode->host);
		  break;
		}
	  ...
	  if ((str != NULL)||(wd->unresolved_timeout>0)){
		...
		if (str){
		  c->ip = pstrdup(p,str);
		  c->resendhost = pstrdup(p,iternode->host);
		}


		SEM_LOCK(wd->cache_sem);
		wpxhash_put(wd->cache_table,c->host,(void *)c);
	  ...
	  while(l != NULL){
		...
		if (temp->resolving == 2)
		  ...
		  if (str)
			dnsrv_resend(temp->packet->x, str, iternode->host);
		  else
			dnsrv_resend(temp->packet->x, NULL, NULL);
		}
	  }
	}
...
void dnsrv_lookup(wpdns wd, dpacket p){
	...
	/* if already in queue */
    if (l != NULL){
	  log_debug("dnsrv: Adding lookup request for %s to pending queue.", p->host);
...
int _dnsrv_beat_packets(void *arg, void *key, void *data){
  ...
  for (l = (dns_packet_list)data;l ; l = l->next){
	...
	if (l->stamp + wd->packet_timeout < now ){
	  ...
	  l->resolving = 2;
	  /* send error */
	  deliver_fail(dpacket_copy(l->packet),"Hostname Resolution Timeout");
	}
  }
...
int _dnsrv_beat_cache(void *arg, void *key, void *data){
  ...
  if ((c->stamp + wd->cache_timeout < now ) ||
	  ((c->ip == NULL) && (c->stamp + wd->unresolved_timeout < now)) ){
...
void dnsrv(instance i, xmlnode x){
     ...
     while (iternode != NULL){
	  ...
	  tmplist->service = pstrdup(wd->p, xmlnode_get_attrib(iternode, "service"));
     ...
     wd->packet_table = wpxhash_new(j_atoi(xmlnode_get_tag_data(config,"queuesize"),101));
     wd->packet_timeout = j_atoi(xmlnode_get_tag_data(config,"queuetimeout"),60);
	 wd->first = NULL;
	 wd->last = NULL;
	 log_alert("config","Queue size: %d, timeout: %d",
			   wd->packet_table->prime,
			   wd->packet_timeout);

     /* Setup the internal hostname cache */
	 SEM_INIT(wd->cache_sem);
     wd->cache_table = wpxhash_new(j_atoi(xmlnode_get_tag_data(config,"cachesize"),1999));
     wd->cache_timeout = j_atoi(xmlnode_get_tag_data(config,"cachetimeout"),3600);
	 wd->unresolved_timeout = j_atoi(xmlnode_get_tag_data(config,"unresolvedtimeout"),30);

	 log_alert("config","Cache size: %d, timeout: %d, uresolved timeout: %d",
			   wd->cache_table->prime,
			   wd->cache_timeout,wd->unresolved_timeout);

	 /* thread count */
	 wd->thread_count = j_atoi(xmlnode_get_attrib(config,"threads"),1);
	 wd->shutdown = 0;

	 log_alert("config","threads %d",
			   wd->thread_count);
	 ...
	 wd->threads->wd = wd;
	 pthread_create(&(wd->threads->t),NULL,wpdns_thread_main,(void*)wd->threads);
