...
#define WPHASH_LIST_BUCKET struct wpxhn_list_struct *wpnext; \
	...
	typedef int (*wpghash_walker) (void *user_data, void *key,
				       void *data);
	typedef void (*wpxhash_walker) (wpxht h, char *key, void *val,
					void *arg);

	typedef void (*wpxhash_list_walker) (wpxht_list h, char *key,
					     void *val, void *arg);

	typedef void (*wpxhash_si_walker) (wpxht_si h, char *key,
					   void *val, void *arg);
	typedef void (*wpxhash_si_walker_i) (wpxht_si h, unsigned int key,
					     void *val, void *arg);
...
/* hash with list */
	...
	void wpxhash_list_walk(wpxht_list h, wpxhash_list_walker w,
			       void *arg);

/* Str int */
	wpxht_si wpxhash_si_new(unsigned int prime);
	wpxht_si wpxhash_si_new_pool(unsigned int prime, pool p);
	int wpxhash_si_put(wpxht_si h, char *str_key, unsigned int int_key,
			   void *val);
	...
	void wpxhash_si_walk_i(wpxht_si h, wpxhash_si_walker_i w,
			       void *arg);
