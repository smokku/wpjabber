...
static xmlnode _xmlnode_append_sibling(xmlnode lastsibling,
			       ...
			       unsigned int type)
{
	...
	/* Otherwise, append this to the lastchild */
		...
		result =
		    _xmlnode_append_sibling(parent->lastchild, name, type);
	}
...
xmlnode xmlnode_get_tag(xmlnode parent, const char *name)
{
	...
	if (strstr(name, "/") == NULL && strstr(name, "?") == NULL
	    && strstr(name, "=") == NULL)
		return _xmlnode_search(parent->firstchild, name,
				       NTYPE_TAG);
	...
	if (equals != NULL && (slash == NULL || equals < slash) && (qmark == NULL || equals < qmark)) {	/* of type =cdata */
		...
		*equals = '\0';
		...
		for (step = parent->firstchild; step != NULL;
			...
			if (*str != '\0')
				if (j_strcmp(xmlnode_get_name(step), str)
				    != 0)
					continue;
...
void xmlnode_put_attrib(xmlnode owner, const char *name, const char *value)
{
	...
	if (owner->firstattrib == NULL) {
		...
		attrib =
		    _xmlnode_search(owner->firstattrib, name,
				    NTYPE_ATTRIB);
		if (attrib == NULL) {
			attrib =
			    _xmlnode_append_sibling(owner->lastattrib,
						    name, NTYPE_ATTRIB);
			owner->lastattrib = attrib;
		}
	}
