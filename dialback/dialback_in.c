...
void dialback_in_read_db(mio m, int flags, void *arg, xmlnode x){
    ...
    switch(flags){
    case MIO_XML_NODE:
	  log_debug("dbin read dialback: fd %d packet %s",m->fd, xmlnode2str(x));
	  ...
	  if((from = jid_new(xmlnode_pool(x),xmlnode_get_attrib(x,"from"))) == NULL || (key = jid_new(xmlnode_pool(x),xmlnode_get_attrib(x,"to"))) == NULL){
		log_alert("error","Invalid packet recived from %s, packet %s, socket %p",
				  c->id, xmlnode2str(x),m);
		mio_write(m, NULL, "<stream:error>Invalid Packets Received!</stream:error>", -1);
		mio_close(m);
		xmlnode_free(x);
		return;
	  }
	...
	return;
	  ...
	  if(md == NULL || md->m != m){ /* dude, what's your problem!  *click* */
		SEM_UNLOCK(c->d->in_sem);
		if (md)
		  log_alert("error"," err md = %p, md->m = %p, m = %p",md,md->m,m);
		else
		  log_alert("error"," null md = %p, m = %p",md,m);
...
void miod_in_cleanup(void *arg){
  ...
  log_record((char *)md->wpkey, "in" , "dialback", "%d %s", md->count, md->m->ip);
...
void dialback_in_verify(db d, xmlnode x){
	...
	SEM_LOCK(d->in_sem);
    if((c = wpxhash_get(d->in_id, xmlnode_get_attrib(x,"id"))) == NULL){
	    SEM_UNLOCK(d->in_sem);
    ...
    if(j_strcmp(xmlnode_get_attrib(x,"type"),"valid") == 0){
	  ...
	  wpxhash_put(c->d->in_ok_db, pstrdup(md->p,jid_full(key)), md);
	}
