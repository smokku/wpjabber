...
void dialback_out_connect(dboc c)
{
	...
	log_debug("Attempting to connect to %s at %s", jid_full(c->key),
		  ip);
	...
	mio_connect(ip, port, dialback_out_read, (void *) c, 20,
		    MIO_CONNECT_XML);
}
...
dboc dialback_out_connection(db d, jid key, char *ip)
{
	...
	/* insert in the hash */
	wpxhash_put(d->out_connecting, pstrdup(p, jid_full(key)),
		    (void *) c);
...
void dialback_out_connection_cleanup(dboc c)
{
	...
	for (x = xmlnode_get_firstchild(c->verifies); x != NULL;
...
void dialback_out_packet(db d, xmlnode x, char *ip)
{
	...
	if (to == NULL || from == NULL) {
		log_warn(d->i->id,
			 "dropping packet, invalid to or from: %s",
			 xmlnode2str(x));
		xmlnode_free(x);
		return;
	}
	...
	if (j_strcmp(from->server, d->i->id) == 0) {
		...
		xmlnode_put_attrib(x, "from",
				   xmlnode_get_attrib(x, "ofrom"));
		...
		if (from == NULL) {
			log_warn(d->i->id,
				 "dropping packet, invalid from: %s",
				 xmlnode2str(x));
			xmlnode_free(x);
			return;
		}
	...
	if (md != NULL) {
		log_debug
		    ("outgoing packet with key %s and located existing %X",
		     jid_full(key), md);
	...
	if (!c) {
		...
		log_warn(key->server,
			 "Unable to connect packet: %s  ip %s ",
			 xmlnode2str(x), ipp);
	}
	...
	if (c == NULL) {
		SEM_UNLOCK(d->out_sem);
		log_warn(d->i->id,
			 "dropping a packet that was missing an ip to connect to: %s",
			 xmlnode2str(x));
		xmlnode_free(x);
		return;
	}
...
void dialback_out_read_db(mio m, int flags, void *arg, xmlnode x)
{
	...
	case MIO_XML_NODE:
		...
		if (j_strcmp(xmlnode_get_name(x), "stream:error") == 0) {
			log_debug("reveived stream error: %s",
				  xmlnode_get_data(x));
		} else {
			mio_write(m, NULL,
				  "<stream:error>Not Allowed to send data on this socket!</stream:error>",
				  -1);
		}
	...
	case MIO_CLOSED:
		...
		/* save ip for logs */
		log_record(md->server, "out", "dialback", "%d %s",
			   md->count, md->m->ip);
...
void dialback_out_read(mio m, int flags, void *arg, xmlnode x)
{
	...
	log_debug("dbout read: fd %d flag %d key %s", m->fd, flags,
		  jid_full(c->key));

	switch (flags) {
	case MIO_NEW:
		log_debug("NEW outgoing server socket connected at %d",
			  m->fd);

		/* outgoing connection, write the header */
		cur =
		    xstream_header("jabber:server", c->key->server, NULL);
	...
	case MIO_XML_ROOT:
		...
		SEM_LOCK(c->d->in_sem);
		if (wpxhash_get(c->d->in_id, xmlnode_get_attrib(x, "id"))
		    != NULL) {
			SEM_UNLOCK(c->d->in_sem);
			log_alert(c->key->server,
				  "hostname maps back to ourselves!");
			mio_write(m, NULL,
				  "<stream:error>Mirror Mirror on the wall</stream:error>",
				  -1);
			mio_close(m);
			break;
		}
		...
		xmlnode_insert_cdata(cur,
				     dialback_merlin(xmlnode_pool(cur),
						     c->d->secret,
						     c->key->server,
		...
		c->m = m;
		for (cur = xmlnode_get_firstchild(c->verifies);
	...
	case MIO_XML_NODE:
		...
		if (j_strcmp(xmlnode_get_name(x), "db:result") == 0) {
			if (j_strcmp(xmlnode_get_attrib(x, "from"), c->key->server) != 0 || j_strcmp(xmlnode_get_attrib(x, "to"), c->key->resource) != 0) {	/* naughty... *click* */
				log_warn(c->d->i->id,
					 "Received illegal dialback validation remote %s != %s or to %s != %s",
					 c->key->server,
					 xmlnode_get_attrib(x, "from"),
					 c->key->resource,
					 xmlnode_get_attrib(x, "to"));
				mio_write(m, NULL,
					  "<stream:error>Invalid Dialback Result</stream:error>",
					  -1);
				mio_close(m);
				break;
			}
			...
			if (j_strcmp
			    (xmlnode_get_attrib(x, "type"),
			     "valid") == 0) {
				SEM_LOCK(c->d->out_sem);

				md = dialback_miod_new(c->d, m, NULL);	/* set up the mio wrapper */
				log_debug
				    ("miod registering out connection: socket %d key %s",
				     md->m->fd, jid_full(c->key));
				md->server =
				    pstrdup(md->p, c->key->server);

				wpxhash_put(c->d->out_ok_db,
					    pstrdup(md->p,
						    jid_full(c->key)), md);
				...
				/* set socket */
				mio_reset(m, dialback_out_read_db,
					  (void *) md);
			...
			log_alert(c->d->i->id,
				  "We were told by %s that our sending name %s is invalid, either something went wrong on their end, we tried using that name improperly, or dns does not resolve to us",
				  c->key->server, c->key->resource);
			mio_write(m, NULL,
				  "<stream:error>I guess we're trying to use the wrong name, sorry</stream:error>",
				  -1);
			mio_close(m);
			break;
		}
		...
		log_warn(c->d->i->id,
			 "Dropping connection due to illegal incoming packet on an unverified socket from %s to %s (%s): %s",
			 c->key->resource, c->key->server, m->ip,
			 xmlnode2str(x));
		mio_write(m, NULL,
			  "<stream:error>Not Allowed to send data on this socket!</stream:error>",
			  -1);
...
int _dialback_out_beat_packets(void *arg, void *key, void *data)
{
	...
	while (cur != NULL) {
		...
		mtq_send(NULL, NULL, __dialback_deliver_fail,
			 (void *) cur->x);
...
result dialback_out_beat_packets(void *arg)
{
	db d = (db) arg;
	SEM_LOCK(d->out_sem);
	wpghash_walk(d->out_connecting, _dialback_out_beat_packets,
		     (void *) time(NULL));
	SEM_UNLOCK(d->out_sem);
	return r_DONE;
}
