...
char *dialback_merlin(pool p, char *secret, char *to, char *challenge)
{
	...
	log_debug("merlin casts his spell(%s+%s+%s) %s", secret, to,
		  challenge, res);
	return res;
}
...
void dialback_miod_read(miod md, xmlnode x)
{
	...
	if (jp == NULL) {
		log_warn(md->d->i->id,
			 "dropping invalid packet from server: %s",
			 xmlnode2str(x));
		xmlnode_free(x);
		return;
	}
...
void _dialback_beat_idle(wpxht h, char *key, void *val, void *arg)
{
	...
	if (((int) *(time_t *) arg - md->last) >= md->d->timeout_idle) {
		log_debug("Idle Timeout on socket %d to %s", md->m->fd,
			  md->m->ip);
		mio_close(md->m);
	}
}
...
void dialback(instance i, xmlnode x)
{
	...
	d->timeout_idle =
	    j_atoi(xmlnode_get_tag_data(cfg, "idletimeout"), 900);
	d->timeout_packets =
	    j_atoi(xmlnode_get_tag_data(cfg, "queuetimeout"), 30);
	...
	d->cache_max_timeout =
	    j_atoi(xmlnode_get_tag_data(cfg, "cachemaxtimeout"),
		   3600 * 24 * 7);
	...
	if ((cur = xmlnode_get_tag(cfg, "karma")) != NULL) {
		k.val =
		    j_atoi(xmlnode_get_tag_data(cur, "init"), KARMA_INIT);
		k.max =
		    j_atoi(xmlnode_get_tag_data(cur, "max"), KARMA_MAX);
		k.inc =
		    j_atoi(xmlnode_get_tag_data(cur, "inc"), KARMA_INC);
		k.dec =
		    j_atoi(xmlnode_get_tag_data(cur, "dec"), KARMA_DEC);
		k.restore =
		    j_atoi(xmlnode_get_tag_data(cur, "restore"),
			   KARMA_RESTORE);
		k.penalty =
		    j_atoi(xmlnode_get_tag_data(cur, "penalty"),
			   KARMA_PENALTY);
		k.reset_meter =
		    j_atoi(xmlnode_get_tag_data(cur, "resetmeter"),
			   KARMA_RESETMETER);
	...
	if ((cur = xmlnode_get_tag(cfg, "ip")) != NULL)
		for (; cur != NULL;
		     xmlnode_hide(cur), cur = xmlnode_get_tag(cfg, "ip")) {
			mio m;
			m = mio_listen(j_atoi
				       (xmlnode_get_attrib(cur, "port"),
					5269), xmlnode_get_data(cur),
				       dialback_in_read, (void *) d,
				       MIO_LISTEN_XML);
			if (m == NULL)
				return;
		...
		mio m;
		m = mio_listen(5269, NULL, dialback_in_read, (void *) d,
			       MIO_LISTEN_XML);
		if (m == NULL)
			return;
		/* Set New rate and points */
	...
	register_beat(d->timeout_packets, dialback_out_beat_packets,
		      (void *) d);
