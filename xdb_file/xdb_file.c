...
result xdb_file_purge(void *arg)
{
	...
	for (c = xf->last; c != NULL;) {
		...
		if (c->ref)
			break;
		if (c->lasttime > xf->actualtime - xf->timeout)
			break;
...
/* extern */
char *xdb_file_full(int create, pool p, char *spl, char *host, char *file,
		    char *ext);
...
result xdb_file_phandler(instance i, dpacket dp, void *arg)
{
	...
	if (full == NULL) {
		...
		if (flag_set == 0) {
			...
			xmlnode_put_attrib(dp->x, "to",
					   xmlnode_get_attrib(dp->x,
							      "from"));
			xmlnode_put_attrib(dp->x, "from",
					   jid_full(dp->id));
	...
	if (flag_set == 1) {
		...
		if (act != NULL) {
			...
			case 'i':
				...
				if (data == NULL) {
					data =
					    xmlnode_insert_tag(top, "foo");
					xmlnode_put_attrib(data, "xdbns",
							   ns);
			...
			case 'c':	/* check action */
				if (match != NULL)
					data =
					    xmlnode_get_tag(data, match);
				if (j_strcmp
				    (xmlnode_get_data(data),
				     xmlnode_get_data
				     (xmlnode_get_firstchild(dp->x))) !=
				    0) {
					log_debug
					    ("xdb check action returning error to signify unsuccessful check");
					ret = 0;
				}
				flag_set = 0;
				break;
			...
			default:
				log_warn("xdb_file",
					 "unable to handle unknown xdb action '%s'",
					 act);
				ret = 0;
				break;
			}
		...
		if (ret) {
			...
			if (flag_set) {
				int result =
				    xdb_file2file((char *) c->wpkey, file,
						  xf->sizelimit);

				if (result < 0) {
					log_error(dp->id->server,
			...
			if (ret) {
				xmlnode_put_attrib(dp->x, "type",
						   "result");
				xmlnode_put_attrib(dp->x, "to",
						   xmlnode_get_attrib(dp->
								      x,
								      "from"));
				xmlnode_put_attrib(dp->x, "from",
						   jid_full(dp->id));
	...
	/* =================================  G E T  =================================== */
		...
		if (data != NULL) {	/* cool, send em back a copy of the data */
			xmlnode_hide_attrib(xmlnode_insert_tag_node
					    (dp->x, data), "xdbns");
		}
...
void xdb_file(instance i, xmlnode x)
{
	...
	if (spl == NULL) {
		log_error(NULL,
			  "xdb_file: No filesystem spool location configured");
		return;
	}
	...
	xf->hash =
	    wpxhash_new(j_atoi
			(xmlnode_get_tag_data(config, "maxfiles"),
			 FILES_PRIME));

	log_debug("Starting XDB size limit =  %d, timeout = %d", sizelimit,
		  timeout);
