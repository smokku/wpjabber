...
result base_file_deliver(instance id, dpacket p, void *arg)
{
	...
	if ((ltime > bf->last_time) || (bf->f == NULL)) {
		...
		if ((bf->yesterday.tm_mday != today.tm_mday)
		    || (bf->f == NULL)) {
			memcpy(&(bf->yesterday), &today,
			       sizeof(struct tm));
	...
	if (bf->f == NULL) {
		log_debug
		    ("base_file_deliver error: no file available to print to.\n");
		return r_ERR;
	}
	...
	if (message == NULL) {
		log_debug
		    ("base_file_deliver error: no message available to print.\n");
		return r_ERR;
	}

	if (fprintf(bf->f, "%s\n", message) == EOF) {
		log_debug
		    ("base_file_deliver error: error writing to file(%d).\n",
		     errno);
		return r_ERR;
	}
	fflush(bf->f);
...
result base_file_config(instance id, xmlnode x, void *arg)
{
	...
	if (id == NULL) {
		log_debug("base_file_config validating configuration");

		if (xmlnode_get_data(x) == NULL) {
			log_debug
			    ("base_file_config error: no filename provided.");
			xmlnode_put_attrib(x, "error",
					   "'file' tag must contain a filename to write to");
			return r_ERR;
		}
		return r_PASS;
	}

	log_debug("base_file configuring instance %s", id->id);

	if (id->type != p_LOG) {
		log_alert(NULL,
			  "ERROR in instance %s: <file>..</file> element only allowed in log sections",
			  id->id);
		return r_ERR;
	}
