...
wpxht_si wpxhash_si_new(unsigned int prime)
{
	...
	xnew =
	    (wpxht_si) malloc(sizeof(_wpxht_si) +
			      (sizeof(wpxhn_si) * prime * 2));
	memset(xnew, 0,
	       sizeof(_wpxht_si) + (sizeof(wpxhn_si) * prime * 2));
	xnew->prime = prime;
	xnew->zen_str =
	    (wpxhn_si *) ((unsigned char *) xnew + sizeof(_wpxht_si));
	xnew->zen_int =
	    (wpxhn_si *) ((unsigned char *) xnew + sizeof(_wpxht_si) +
...
wpxht_list wpxhash_list_new(unsigned int prime)
{
	...
	xnew =
	    (wpxht_list) malloc(sizeof(_wpxht_list) +
				sizeof(wpxhn_list) * prime);
	memset(xnew, 0, sizeof(_wpxht_list) + sizeof(wpxhn_list) * prime);
	xnew->prime = prime;
	xnew->zen =
	    (wpxhn_list *) ((unsigned char *) xnew + sizeof(_wpxht_list));
	xnew->first = NULL;
	xnew->last = NULL;
	return xnew;
}
