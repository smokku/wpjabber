...
void do_include(int nesting_level, xmlnode x)
{
	...
	for (; cur != NULL;) {
		...
		if (j_strcmp(xmlnode_get_name(cur), "jabberd:include") ==
		    0) {
			xmlnode include;
			...
			if (nesting_level > MAX_INCLUDE_NESTING) {
				fprintf(stderr,
					"ERROR: Included files nested %d levels deep.  Possible Recursion\n",
					nesting_level);
				exit(1);
			}
			include = cur;
			 ...
			 */
			if (j_strcmp
			    (xmlnode_get_name(xmlnode_get_parent(cur)),
			     xmlnode_get_name(include_x)) == 0)
				xmlnode_insert_node(x,
...
void cmdline_replace(xmlnode x)
{
	...
	for (; cur != NULL; cur = xmlnode_get_nextsibling(cur)) {
		if (cur->type != NTYPE_TAG)
			continue;
		if (j_strcmp(xmlnode_get_name(cur), "jabberd:cmdline") !=
		    0) {
			cmdline_replace(cur);
			continue;
		}
		...
		if (replace_text == NULL)
			replace_text = xmlnode_get_data(cur);
...
int configurate(char *file)
{
	...
	if (greymatter__ == NULL)
		greymatter__ =
		    xmlnode_str(SINGLE_CONFIG, strlen(SINGLE_CONFIG));
#endif
...
cfg cfget(char *node)
{
	...
	for (next = cfhandlers__;
	     next != NULL && strcmp(node, next->node) != 0;
	     next = next->next);
...
int instance_startup(xmlnode x, int exec)
{
	...
	if (type == p_NONE || xmlnode_get_attrib(x, "id") == NULL
	    || xmlnode_get_firstchild(x) == NULL) {
		...
		if (xmlnode_get_attrib(x, "id") == NULL) {
			fprintf(stderr,
				"ERROR: Section needs an 'id' attribute\n");
		}
		if (xmlnode_get_firstchild(x) == NULL) {
			fprintf(stderr,
				"ERROR: Section Has no data in it\n");
		}
		return -1;
	}
	...
	if (exec == 1) {
		newi =
		    ghash_get(instance__ids, xmlnode_get_attrib(x, "id"));
		if (newi != NULL) {
			fprintf(stderr,
				"ERROR: Multiple Instances with same id: %s\n",
	...
	for (cur = xmlnode_get_firstchild(x); cur != NULL;
		...
		if (c == NULL || (c->f) (newi, cur, c->arg) == r_ERR) {
			char *error =
			    pstrdup(xmlnode_pool(cur),
				    xmlnode_get_attrib(cur, "error"));
			xmlnode_hide_attrib(cur, "error");
			fprintf(stderr,
				"Invalid Configuration in instance '%s':\n%s\n",
				xmlnode_get_attrib(x, "id"),
				xmlnode2str(cur));
			if (c == NULL)
				fprintf(stderr,
					"ERROR: Unknown Base Tag: %s\n",
...
int configo(int exec)
{
	...
	if (instance__ids == NULL)
		instance__ids =
		    ghash_create_pool(jabberd__runtime, 19,
				      (KEYHASHFUNC) str_hash_code,
				      (KEYCOMPAREFUNC) j_strcmp);

	for (cur = xmlnode_get_firstchild(greymatter__); cur != NULL;
	     cur = xmlnode_get_nextsibling(cur)) {
		if (xmlnode_get_type(cur) != NTYPE_TAG
		    || strcmp(xmlnode_get_name(cur), "base") == 0)
			continue;
