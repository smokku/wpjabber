...
result base_format_modify(instance id, dpacket p, void *arg)
{
	...
	while (nxt != NULL) {
		...
		case 't':
			...
			break;
		case 'd':
			spooler(log_result, jutil_timestamplocal(),
				log_result);
			break;
		case 's':
			spooler(log_result, xmlnode_get_data(p->x),
				log_result);
			break;
		default:
			log_debug("Invalid argument: %s", nxt[0]);
		}
...
{
	if (id == NULL) {
		log_debug("base_format_config validating configuration");
		if (xmlnode_get_data(x) == NULL) {
			log_debug("base_format invald format");
	...
	log_debug("base_format configuring instance %s ", id->id);

	if (id->type != p_LOG) {
		log_alert(NULL,
			  "ERROR in instance %s: <format>..</format> element only allowed in log sections",
			  id->id);
		return r_ERR;
	}
