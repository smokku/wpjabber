...
void base_connect_process_xml(mio m, int state, void* arg, xmlnode x){
	...
	case MIO_NEW:
	    ...
	    log_debug("base_connecting: %X, %X, %s", ci, ci->inst, ci->inst->id);
	    ...
	    /* Only deliver packets after the connection is auth'd */
		  if (ci->state == conn_AUTHD){
			  if (j_strcmp(xmlnode_get_name(x), "endconnection") == 0){
				log_alert("connect","Server is closing. stop sending to him");
				xmlnode_free(x);
	...
	case MIO_CLOSED:
	    ...
	    if(ci->tries_left == 0){
		log_alert("connect", "Base Connect Failed: service %s was unable to connect to %s:%d, unrecoverable error, exiting", ci->inst->id, ci->hostip, ci->hostport);
		return;
	    }

	    /* pause 2 seconds, and reconnect */
	    log_debug("Base Connect Failed to connect to %s:%d Retry [%d] in 2 seconds...", ci->hostip, ci->hostport, ci->tries_left);
...
result base_connect_config(instance id, xmlnode x, void *arg){
	...
	if (ci->xdb_rethread){
	  log_alert("connect","xdb requests will be moved to mtq thread in connect instance %s",
				id->id);
	}
