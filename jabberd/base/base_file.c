...
result base_file_deliver(instance id, dpacket p, void* arg){
  ...
  if (bf->f == NULL){
      log_debug("base_file_deliver error: no file available to print to.\n");
      return r_ERR;
    }

  message = xmlnode_get_data(p->x);
  if (message == NULL){
      log_debug("base_file_deliver error: no message available to print.\n");
      return r_ERR;
  }

  if (fprintf(bf->f,"%s\n", message) == EOF){
	log_debug("base_file_deliver error: error writing to file(%d).\n", errno);
	return r_ERR;
  }
  fflush(bf->f);
...
result base_file_config(instance id, xmlnode x, void *arg){
	...
	if (xmlnode_get_data(x) == NULL){
	    log_debug("base_file_config error: no filename provided.");
