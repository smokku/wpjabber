...
void base_connect_queue(conn_info ci, xmlnode x)
{
	...
	log_debug("insert packet into queue");
...
void base_connect_connect(void *arg)
{
	...
	mio_connect(ci->hostip, ci->hostport, base_connect_process_xml,
		    (void *) ci, ci->timeout, NULL, mio_handlers_new(NULL,
								     NULL,
								     MIO_XML_PARSER));
}
...
void base_connect_process_xml(mio m, int state, void *arg, xmlnode x)
{
	...
	case MIO_NEW:
		...
		log_debug("base_connecting: %X, %X, %s", ci, ci->inst,
			  ci->inst->id);

		cur =
		    xstream_header("jabber:component:accept", ci->inst->id,
				   NULL);
	...
	case MIO_XML_ROOT:
		...
		shahash_r(spools
			  (x->p, xmlnode_get_attrib(x, "id"), ci->secret,
			   x->p), hashbuf);
	...
	case MIO_XML_NODE:
		...
		if (ci->state == conn_AUTHD) {
			if (j_strcmp(xmlnode_get_name(x), "endconnection")
			    == 0) {
				log_alert("connect",
				...
				if (ci->event != NULL) {
					...
					log_debug("event disconnection");

					cur = xmlnode_new_tag("route");
					xmlnode_put_attrib(cur, "type",
							   "disconnected");
					xmlnode_put_attrib(cur, "to",
							   ci->event);
					xmlnode_put_attrib(cur, "from",
							   ci->inst->id);
					deliver(dpacket_new(cur),
						ci->inst);
				}
				...
				if ((ci->dplast) && (ci->xdb_rethread)
				    && (ci->dplast->type == p_XDB)) {
					/* XXX mtq_send */
	...
	case MIO_CLOSED:
		...
		if (ci->tries_left == 0) {
			log_alert("connect",
				  "Base Connect Failed: service %s was unable to connect to %s:%d, unrecoverable error, exiting",
				  ci->inst->id, ci->hostip, ci->hostport);
			return;
		}

		/* pause 2 seconds, and reconnect */
		log_debug
		    ("Base Connect Failed to connect to %s:%d Retry [%d] in 2 seconds...",
		     ci->hostip, ci->hostport, ci->tries_left);
		mtq_send(NULL, ci->mempool, base_connect_connect,
			 (void *) ci);
...
result base_connect_config(instance id, xmlnode x, void *arg)
{
	...
	if (id == NULL) {
		log_debug("base_accept_config validating configuration\n");
		if (port == 0 || (secret == NULL)) {
			xmlnode_put_attrib(x, "error",
					   "<connect> requires the following subtags: <port>, and <secret>");
			return r_ERR;
		}
		return r_PASS;
	}

	log_debug("Activating configuration: %s\n", xmlnode2str(x));
	...
	if (ci->xdb_rethread) {
		log_alert("connect",
			  "xdb requests will be moved to mtq thread in connect instance %s",
			  id->id);
	}
	...
	register_phandler(id, o_DELIVER, base_connect_deliver,
			  (void *) ci);
...
{
	log_debug("base_connect loading...\n");
