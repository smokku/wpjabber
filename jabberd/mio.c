...
int _mio_write_dump(mio m)
{
	...
	while (m->queue != NULL) {
		...
		log_debug("write_dump writing data: %.*s", cur->len,
			  cur->cur);
...
mio _mio_accept(mio m)
{
	...
	fd = (*m->mh->accept) (m, (struct sockaddr *) &serv_addr,
			       &addrlen);
	if (fd <= 0) {
		return NULL;
	}
	...
	if (deny >= allow) {
		log_warn("mio",
			 "%s was denied access, due to the allow list of IPs",
	...
	new =
	    mio_new(fd, m->cb, m->cb_arg,
		    mio_handlers_new(m->mh->read, m->mh->write,
				     m->mh->parser));
...
void *_mio_connect(void *arg)
{
	...
	log_debug("Connecting on socket %d host %s [%d]", new->fd, cd->ip,
		  getpid());
	...
	if (new->fd < 0
		...
		if (cd->cb != NULL)
			(*(mio_std_cb) cd->cb) (new, MIO_CLOSED,
						cd->cb_arg);
		cd->connected = -1;
	...
	if (cd->bind) {
		...
		if (bind
		    (new->fd, (struct sockaddr *) &sa,
		     sizeof(struct sockaddr_in)) < 0) {
			log_alert("bind", "Error %d", errno);
		}
	} else if (xmlnode_get_tag_data(greymatter__, "io/bind") != NULL) {
		memset((void *) &sa, 0, sizeof(struct sockaddr_in));
		sa.sin_family = AF_INET;
		sa.sin_port = 0;
		inet_aton(xmlnode_get_tag_data(greymatter__, "io/bind"),
			  &sa.sin_addr);
		if (bind
		    (new->fd, (struct sockaddr *) &sa,
		     sizeof(struct sockaddr_in)) < 0) {
	...
	if (n != 0) {
		...
		if ((n =
		     select(new->fd + 1, &rset, &wset, NULL, &tv)) == 0) {
			ret = -1;
		} else {

			if (FD_ISSET(new->fd, &rset)
			    || FD_ISSET(new->fd, &wset)) {
	...
	mio_karma(new, KARMA_DEF_INIT, KARMA_DEF_MAX, KARMA_DEF_INC,
...
void *_mio_main(void *arg)
{
	...
	while (1) {
		...
		for (cur = mio__data->master__list; cur != NULL;) {
			...
			if ((karma == 1) && (cur->state != state_CLOSE)
			    && cur->k.val != KARMA_DEF_INIT) {
				karma_increment(&(cur->k));

				if (!FD_ISSET(cur->fd, &all_rfds)
				    && cur->k.val >= 0) {
					log_debug
					    ("socket %d has restore karma %d -=> %d",
					     cur->fd, cur->k.val,
					     cur->k.restore);
			...
			if (cur->k.val == KARMA_DEF_INIT) {
				log_debug
				    ("socket %d has restore karma %d -=> %d",
				     cur->fd, cur->k.val, cur->k.restore);
				/* reset the karma to restore val */
				cur->k.val = cur->k.restore;
			...
			} else if ((cur->queue != NULL)
				   && (!FD_ISSET(cur->fd, &all_wfds))) {
				FD_SET(cur->fd, &all_wfds);
			}
...
void mio_init(void)
{
	...
	KARMA_DEF_INIT =
	    j_atoi(xmlnode_get_tag_data(io, "karma/init"), KARMA_INIT);
	KARMA_DEF_MAX =
	    j_atoi(xmlnode_get_tag_data(io, "karma/max"), KARMA_MAX);
	KARMA_DEF_INC =
	    j_atoi(xmlnode_get_tag_data(io, "karma/inc"), KARMA_INC);
	KARMA_DEF_DEC =
	    j_atoi(xmlnode_get_tag_data(io, "karma/dec"), KARMA_DEC);
	KARMA_DEF_PENALTY =
	...
	KARMA_DEF_RATE_T =
	    j_atoi(xmlnode_get_attrib(xmlnode_get_tag(io, "rate"), "time"),
		   5);
	KARMA_DEF_RATE_P =
	    j_atoi(xmlnode_get_attrib
		   (xmlnode_get_tag(io, "rate"), "points"), 25);
	...
	if (mio__data == NULL) {
		...
		mio__data->karma_time =
		    j_atoi(xmlnode_get_tag_data(io, "heartbeat"),
			   KARMA_HEARTBEAT) * 1000;
...
void mio_stop(void)
{
	...
	for (cur = mio__data->master__list; cur != NULL;) {
		...
		if (setsockopt
		    (temp->fd, SOL_SOCKET, SO_LINGER, (void *) &ling,
		     sizeof(ling)) < 0) {
...
void mio_write(mio m, xmlnode x, char *buffer, int len)
{
	...
	log_debug("mio_write called on x: %X buffer: %.*s", x, len,
		  buffer);
...
/*
   sets karma values
*/
void mio_karma(mio m, int val, int max, int inc, int dec, int penalty,
	       int restore)
{
	if (m == NULL)
		return;
...
void mio_connect_bind(char *host, int port, void *cb, void *cb_arg,
		      int timeout, mio_connect_func f, mio_handlers mh,
		      char *bind)
{
...
void mio_connect(char *host, int port, void *cb, void *cb_arg, int timeout,
		 mio_connect_func f, mio_handlers mh)
{
	connect_data cd = NULL;
	pool p = NULL;
	pthread_attr_t attr;
 ...
 */
mio mio_listen(int port, char *listen_host, void *cb, void *arg,
	       mio_accept_func f, mio_handlers mh)
{
	mio new;
	int fd;
	...
	if (fd < 0) {
		log_alert("mio",
			  "unable to open listening socket on %d [%s] - %s",
	...
	if (listen(fd, 10) < 0) {
		log_alert("mio", "unable to listen on %d [%s] - %s", port,
			  listen_host, strerror(errno));
		return NULL;
	}
...
void mio_clean()
{
	...
	while (l != 0) {
		...
		for (cur = mio__data->master__list; cur != NULL;) {
			...
			if (cur->state != state_CLOSE) {
				int free_size = -1;
				if (ioctl(cur->fd, FIONREAD, &free_size) ==
				    0) {
					bufer += free_size;
				}
			}
