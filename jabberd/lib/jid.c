...
typedef struct _jid_prep_entry_st {
	...
	time_t last_used;
			/**< when this result has last been successfully used */
	unsigned int used_count;
			     /**< how often this result has been successfully used */
...
typedef struct _jid_prep_cache_st {
	...
	pth_mutex_t mutex;
			/**< mutex controling the access to the hashtable */
...
void _jid_clean_single_cache(_jid_prep_cache_t cache, time_t keep_newer_as)
{
	...
	xhash_walk(cache->hashtable, _jid_clean_walker,
		   (void *) &keep_newer_as);
 ...
 */
int _jid_cached_stringprep(char *in_out_buffer, int max_len,
			   _jid_prep_cache_t cache)
{
	...
	preped =
	    (_jid_prep_entry_t) xhash_get(cache->hashtable, in_out_buffer);
	if (preped != NULL) {
		...
		result =
		    stringprep(in_out_buffer, max_len,
			       STRINGPREP_NO_UNASSIGNED, cache->profile);
		...
		if (result == STRINGPREP_OK && original != NULL) {
			...
			preped =
			    (_jid_prep_entry_t)
			    malloc(sizeof(struct _jid_prep_entry_st));
			if (preped != NULL) {
				...
				pth_mutex_acquire(&(cache->mutex), FALSE,
						  NULL);

				/* store the entry in the cache */
				xhash_put(cache->hashtable, original,
					  preped);
 ...
 */
void _jid_init_single_cache(_jid_prep_cache_t * cache, int prime,
			    const Stringprep_profile * profile)
{
	...
	if (*cache == NULL) {
		*cache =
		    (_jid_prep_cache_t)
		    malloc(sizeof(struct _jid_prep_cache_st));
...
void jid_init_cache()
{
	...
	_jid_init_single_cache(&_jid_prep_cache_node, 2003,
			       stringprep_xmpp_nodeprep);

	/* init the nameprep cache (domains) */
	_jid_init_single_cache(&_jid_prep_cache_domain, 2003,
			       stringprep_nameprep);

	/* init the resourceprep cache */
	_jid_init_single_cache(&_jid_prep_cache_resource, 2003,
...
int _jid_safe_domain(jid id)
{
	...
	result =
	    _jid_cached_stringprep(id->server, strlen(id->server) + 1,
				   _jid_prep_cache_domain);
	if (result == STRINGPREP_TOO_SMALL_BUFFER) {
		...
		result =
		    _jid_cached_stringprep(biggerbuffer, biggerbuffersize,
					   _jid_prep_cache_domain);
...
int _jid_safe_node(jid id)
{
	...
	/* nodeprep */
	result =
	    _jid_cached_stringprep(id->user, strlen(id->user) + 1,
...
int _jid_safe_resource(jid id)
{
	...
	result =
	    _jid_cached_stringprep(id->resource, strlen(id->resource) + 1,
				   _jid_prep_cache_resource);
	if (result == STRINGPREP_TOO_SMALL_BUFFER) {
		...
		result =
		    _jid_cached_stringprep(id->resource,
					   strlen(id->resource) + 1,
					   _jid_prep_cache_resource);
...
int jid_cmpx(jid a, jid b, int parts)
{
	...
	if (parts & JID_USER && _jid_nullstrcasecmp(a->user, b->user) != 0)
		return -1;
	if (parts & JID_SERVER
	    && _jid_nullstrcmp(a->server, b->server) != 0)
		return -1;
