...
void xdb_deliver(instance i, xdbrequest_p cur, int another_thread){
    ...
    if (another_thread){
      xdb_resend xr = pmalloco(xmlnode_pool(x), sizeof(_xdb_resend));
      xr->x = x;
      xr->i = i;
...
xdbcache xdb_cache(instance id){
    ...
    if(id == NULL){
	    log_alert("xdb","Programming Error: xdb_cache() called with NULL\n");
	    return NULL;
    }
...
xmlnode xdb_get(xdbcache xc, jid owner, char *ns){
    ...
    if (xc->shutdown) return NULL;

    if(xc == NULL || owner == NULL || ns == NULL){
	  if (ns){
		log_alert("xdb_get","Programming Error: xdb_get() called with NULL ns: %s\n",ns);
	  }
	  else{
		log_alert("xdb_get","Programming Error: xdb_get() called with NULL ns: NULL\n");
	  }
    ...
    while (cur->preblock != 1) usleep(10);
...
int xdb_act(xdbcache xc, jid owner, char *ns, char *act, char *match, xmlnode data){
    ...
    if (xc->shutdown) return 1;

    if(xc == NULL || owner == NULL || ns == NULL){
      log_alert("xdb_set","Programming Error: xdb_set() called with NULL\n");
      return 1;
    }
