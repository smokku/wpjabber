...
void server_process_xml(io m, int state, void* arg, xmlnode x){
	...
	case IO_NEW:
	    ...
	    log_debug("base_connecting to %s: ",si->mhost);
			...
			cur = xstream_header("jabber:component:accept", wpj->cfg->hostname, NULL);
	...
	case IO_XML_ROOT:
	    ...
	    shahash_r(spools(x->p, xmlnode_get_attrib(x, "id"), si->secret, x->p), hashbuf);
	...
	case IO_XML_NODE:
	    /* Only deliver packets after handshake */
		  if (si->state == conn_AUTHD || si->state == conn_CLOSING){
			  ...
			  if (j_strcmp(xmlnode_get_name(x), "route") == 0){
				...
				x = xmlnode_get_firstchild(rt);
				  ...
				  if (j_strcmp(xmlnode_get_name(x), "endconnection") == 0){
					log_debug("Server is closing. stop sending to him");

					si->state = conn_CLOSING;

					for ( socket = io__data->master__list; socket != NULL; ){
					  ...
					  if ((ci->state == state_AUTHD)
						...
						/* set status refresh */
						ci->state = state_REFRESHING_SESSION;
					  }
					  else{
						/* close not authed connections */
						ci->state = state_CLOSING;
						if (socket->accepted){
						  io_write(socket, NULL, "<stream:error>Server temporarily unavailable</stream:error></stream:stream>", -1);
						}
						temp = socket;
						socket = socket->next;
						io_close(temp);
			  ...
			  /* check id */
			  jsto = jid_new(xmlnode_pool(rt),xmlnode_get_attrib(rt, "to"));
			  ...
			  if ((loc < 2) || (loc >= DEF_MAX_CLIENTS)){
				log_alert("server","sends bad data ! loc=%d",loc);
				xmlnode_free(ox);
				return;
			  }
			  ...
			  if (wpj->clients[loc].state == state_CLOSING){
				...
				if (j_strcmp(xmlnode_get_attrib(rt, "type"), "session")==0){
				  log_alert("server","sessions packet for closing user");
				  jutil_tofrom(ox);
				...
				if (j_strcmp(xmlnode_get_attrib(rt, "type"), "sessioncheck") == 0){
				  jutil_tofrom(ox);
				  xmlnode_put_attrib(ox, "type", "error");
				  io_write(si->m, ox, NULL, 0);
				  return;
			  ...
			  if(j_strcmp(wpj->clients[loc].res, jsto->resource)){
				jutil_tofrom(ox);
				xmlnode_put_attrib(ox, "type", "error");
			  ...
			  if (j_strcmp(xmlnode_get_attrib(rt, "type"), "error") == 0){
				xmlnode h;
				xmlnode errx;
				char *err;
				...
				/* if client oki */
				if (wpj->clients[loc].state != state_CLOSING){
				  wpj->clients[loc].state = state_CLOSING;
				  errx = xmlnode_get_tag(rt, "error");
				  err = xmlnode_get_attrib(rt, "error");
				  h = xmlnode_new_tag_pool(xmlnode_pool(rt), "stream:error");
				  xmlnode_insert_cdata(h, err, strlen(err));
				  io_write(wpj->clients[loc].m,h,NULL,0);
			  ...
			  if( (wpj->clients[loc].state == state_UNKNOWN
				   || wpj->clients[loc].state == state_REFRESHING_SESSION)

				  && j_strcmp(xmlnode_get_attrib(rt, "type"), "session") == 0){
				int send_presence = (wpj->clients[loc].state == state_REFRESHING_SESSION);

				wpj->clients[loc].host = jid_new(wpj->clients[loc].m->p,
												 xmlnode_get_attrib(rt, "from"));

				wpj->clients[loc].state = state_AUTHD;

				log_debug("connection authd");

				if (send_presence && wpj->clients[loc].real_user.presence){
				  log_debug("Sending presence copy");

				  client_process_xml(wpj->clients[loc].m, IO_XML_NODE, (void *)&(wpj->clients[loc]),
									 xmlnode_dup(wpj->clients[loc].real_user.presence));
				}

				/* if errro */
				if (wpj->clients[loc].m->state == state_CLOSE){
				  close_client(wpj->clients[loc].m);
				  xmlnode_free(ox);
				  return;
				}
				...
				while(q != NULL){
				  q2 = q->next;
				  client_process_xml(wpj->clients[loc].m, IO_XML_NODE, (void *)&(wpj->clients[loc]),q->x);
				  /* if error */
				  if (wpj->clients[loc].m->state == state_CLOSE){
					wpj->clients[loc].q_start = q2;
					close_client(wpj->clients[loc].m);
					xmlnode_free(ox);
					return;
				  }
			  ...
			  if(( wpj->clients[loc].state == state_UNKNOWN ||
				 ...
				 && j_strcmp(xmlnode_get_attrib(rt, "type"), "auth") == 0){
				char *type;
				char *id;
				...
				if((j_strcmp(type,"result")==0)&&
				   j_strcmp(wpj->clients[loc].auth_id,id)==0){
				  xmlnode sx;

				  /* komunikat rozpoczecia sesji */
				  sx = xmlnode_new_tag("route");
				  xmlnode_put_attrib(sx, "from", wpj->clients[loc].id);
				  xmlnode_put_attrib(sx, "to", jid_full(wpj->clients[loc].orghost));
				  xmlnode_put_attrib(sx, "type", "session");

				  /* SESSION IP SENDING to have it in logs on server */
				  xmlnode_put_attrib(sx,"ip",wpj->clients[loc].m->ip);

				  io_write(si->m, sx, NULL, 0);

				  log_alert("Session","%p [%s] (%d)",
							wpj->clients[loc].m,
							jid_full(wpj->clients[loc].orghost),
							wpj->clients_count);
				}

				/* do not inform client after session refresh */
				if (wpj->clients[loc].state == state_REFRESHING_SESSION)
				  x = NULL;
				else
				  ...
				  xmlnode_put_attrib(x,"ip",wpj->clients[loc].m->ip);
			  }
		  ...
		  if (j_strcmp(xmlnode_get_name(x), "handshake") == 0){
			...
			if (stime != NULL){
			  loc = strlen(stime);
			  if ((si->servertime[0])&&(j_strncmp(stime,si->servertime,loc)==0))
				reconnect = 0;

			  memcpy(si->servertime,stime,loc+1);
			  log_alert("Server","Handshake OK. >%s< %d",si->servertime,reconnect);
			}
			else{
			...
			for ( socket = io__data->master__list; socket != NULL; ){
			  ...
			  if (ci->state == state_REFRESHING_SESSION){
				if (reconnect){
				  ...
				  /* build packet */
				  h = jutil_iqnew(JPACKET__SET, NS_AUTH);
				  q = xmlnode_get_tag(h,"query");
				  xmlnode_insert_cdata(xmlnode_insert_tag(q,"username"),ci->real_user.username,-1);
				  xmlnode_insert_cdata(xmlnode_insert_tag(q,"resource"),ci->real_user.resource,-1);
				  xmlnode_insert_tag(q,"trickypassword");

				  xmlnode_put_attrib(h,"id",ci->auth_id);

				  rt = xmlnode_wrap(h, "route");
				  xmlnode_put_attrib(rt,"from",ci->id); //wpj id
				  xmlnode_put_attrib(rt,"to",jid_full(ci->orghost));
				  xmlnode_put_attrib(rt, "type", "auth");


				  log_debug("tricky authorisation packet created: %s",xmlnode2str(rt));

				  if (si->m->state != state_ACTIVE) break;

				  io_write(si->m,rt,NULL,0);
				}
				else{
				  /* set state */
				  ...
				  while(q != NULL){
					...
					q2 = q->next;
					client_process_xml(ci->m, IO_XML_NODE, ci,q->x);

					/* if error */
					if (ci->m->state == state_CLOSE){
					  ci->q_start = q2;
					  close_client(ci->m);
					  loc = 1;
					  break;
    ...
    case IO_CLOSED:
	  ...
	  log_debug("Reconnect to %s:%d. Retry in 2 seconds...", si->mip, si->mport);
	  io_connect(si->mip, si->mport, server_process_xml, (void*)si,2);
	  return;
    }
}
