...
int karma_increment_lk(struct karma *k, DWORD time)
{
	...
	if ((k->last_update + KARMA_HEARTBEAT_LK > time)
	    && k->last_update != 0)
		goto CHECK;
...
int _io_write_dump_client(io m)
{
	...
	while (m->queue != NULL) {
		...
		log_debug("write_dump client writing data: %.*s", cur->len,
			  cur->cur);
		...
		/* we wrote the entire node, kill it and move on */
			...
			if ((wpj->cfg->no_message_storage == 0) && (cur->x)
			    &&
			    (strncmp
			     (xmlnode_get_name(cur->x), "message",
			      7) == 0)) {
				io_sended isq;
...
int _io_write_dump_server(io m, int *length)
{
	...
	while (m->queue != NULL) {
		...
		log_debug("write_dump serwer writing data: %.*s", cur->len,
			  cur->cur);
...
io _io_accept(io m)
{
	...
	fd = accept(m->fd, (struct sockaddr *) &serv_addr,
		    (socklen_t *) & addrlen);
	if (fd <= 0) {
		return ((void *) -1);
	}
	...
	if (RateCheckAdd(serv_addr.sin_addr)) {
		log_warn("io_select",
			 "%s is being connection rate limited",
	...
	log_debug("new socket accepted (fd: %d, ip: %s, port: %d)", fd,
		  inet_ntoa(serv_addr.sin_addr),
	...
	s = sizeof(int);
	setsockopt(new->fd, SOL_SOCKET, SO_SNDBUF, (char *) &socket_size,
		   s);
	setsockopt(new->fd, SOL_SOCKET, SO_RCVBUF, (char *) &socket_size,
		   s);

	socket_size = 1;
	setsockopt(new->fd, SOL_SOCKET, SO_KEEPALIVE,
		   (char *) &socket_size, s);
...
void *_io_connect(void *arg)
{
	...
	if (new->fd < 0
	    || setsockopt(new->fd, SOL_SOCKET, SO_REUSEADDR,
	...
	if ((n =
	     IO_CONNECT_FUNC(new->fd, (struct sockaddr *) &sa,
			     sizeof sa)) < 0) {
		if (errno != EINPROGRESS) {
			if (cd->cb != NULL)
				(*(io_std_cb) cd->cb) (new, IO_CLOSED,
						       cd->cb_arg);
			cd->connected = -1;
	...
	if (n != 0) {
		...
		if ((n = poll((struct pollfd *) pfdset, 1, 10000)) == 0) {
			...
			if ((pfdset[0].revents & POLLIN)
			    || (pfdset[0].revents & POLLOUT)) {
				s = sizeof(error);
				if (getsockopt
				    (new->fd, SOL_SOCKET, SO_ERROR, &error,
				     &s) < 0)
					ret = -1;
			} else {
				ret = -1;
	...
	io_karma(new, KARMA_DEF_INIT, KARMA_DEF_MAX, KARMA_DEF_INC, 0,
		 KARMA_DEF_PENALTY, KARMA_DEF_RESTORE);
...
int handle_server_read()
{
	...
	if (!io__data->serverio)
		return -1;
	if (io__data->serverio->state != state_ACTIVE)
		return -1;

	len =
	    IO_READ_FUNC(io__data->serverio->fd, buf,
			 SERVER_SOCKET_BUFFERS);
	...
	if (len == 0) {
		...
		buf[len] = '\0';
		log_debug("IO read from serwer socket %d: %s",
			  io__data->serverio->fd, buf);
...
int handle_client_read(io cur)
{
	...
	if (!cur->accepted) {
		...
		if (wpj->cfg->ssl) {
			...
			if ((strstr(buf, "<stream") == NULL)
				...
				if ((*cur->mh->accept) (cur, NULL,
							NULL) != cur->fd) {
					log_alert("error",
						  "error in accepting SSL");
					io_close(cur);
					return -1;
				}
				wpj->clients_ssl++;
	...
	if (maxlen > CLIENT_SOCKET_BUFFER)
		maxlen = CLIENT_SOCKET_BUFFER;
...
void *io_main(void *arg)
{
	...
	while (1) {
		...
		if (io__data->serverio) {
			...
			if (cur->k.val == KARMA_DEF_INIT) {
				...
				if (fdop(cur->fd, ADDFD) < 0) {
					log_alert("error",
						  "unable to add client to epoll set");
				}
			...
			if (time > cur->last_io + 20) {
				...
				if (cur->ping_in_progress == 1) {
					...
					if (ioctl
						...
						log_debug
						    ("server ping	bufor %d last bufor %d",
						     free_size,
						     cur->
						     write_buffer_count);
				...
				/* no ping in progress */
					...
					if (cur->queue) {
						log_debug("ping server",
							  "send ping queue");
						i = _io_write_dump_server
						    (cur, &length);
						if (i < 0) {
							io_close(cur);
							continue;
			...
			/* server ping */
		} else if (io__data->refresh_flag == 1) {
			/* 5 minut to refresh sessions */
			if (time >
			    io__data->refresh_start +
			    wpj->cfg->reconnect_time) {
				for (cur = io__data->master__list;
				     cur != NULL;) {
					if (wpj->clients[cur->pollpos].
					    state ==
		...
		if (iocheck) {
			...
			for (cur = io__data->master__list; cur != NULL;) {
				...
				if (wpj->clients[cur->pollpos].state ==
				    state_UNKNOWN) {
					if (time >
					    wpj->clients[cur->pollpos].
					    time + wpj->cfg->auth_time) {
						if (wpj->
						    clients[cur->pollpos].
						    orghost) {
							log_alert("auth",
								  "auth time exceeded %s",
								  jid_full
								  (wpj->
								   clients
								   [cur->
					...
					if (cur->s_queue) {
						...
						for (s_cur = cur->s_queue;
							...
							if (time >
							    ...
							    iocheck) {
								log_debug
								    ("packet queue remove time %d now %d",
								     s_cur->
								     t,
								     time);
					...
					if (time >
					    ...
					    wpj->cfg->ping_time) {
						log_alert("ping",
							  "%p time to ping last io: %d",
							  cur,
							  cur->last_io);

						log_debug
						    ("ping check in socket");
						/* if ping was sent */
						if (cur->
						    ping_in_progress ==
						    1) {
							...
							if (ioctl
								...
								log_debug
								    ("ping check in progress bufor %d my bufor %d",
								     free_size,
								     cur->
								     write_buffer_count);
								...
								if ((free_size == 0) || (free_size != cur->write_buffer_count)) {
									log_alert
									    ("ping",
									     "%p buffer has changed, reset ping",
									     cur);
									cur->
									    last_io
									    =
							...
							if (cur->queue) {
								log_alert
								    ("ping",
								     "%p send ping from queue",
								     cur);
								_io_write_dump_client
								    (cur);
							} else {
								/* send ping */
								log_debug
		...
		if (datacheck) {
			...
			if (!wpj->overload)
				for (cur = io__data->data__list;
				     cur != NULL;) {
					log_debug
					    ("socket %d in data__list",
					     cur->fd);
					if (karma_increment_lk(&cur->k, time)) {	/* if karma > 0 */
						if (handle_client_read(cur)
						    == -1) {
							temp = cur;
							cur = cur->next;
							close_client(temp);
		...
		for (i = 0; i < nfds; i++) {
			...
			if ((io__data->serverio)
			    && (io__data->evp[i].data.fd ==
				io__data->serverio->fd)) {
				...
				&& (io__data->evp[i].data.fd ==
				    io__data->listenio->fd)) {
				/* accept new connection */
				cur = NULL;
				while (cur != ((void *) -1)) {

					cur =
					    _io_accept(io__data->listenio);
					...
					if (cur != ((void *) -1)) {	/* if more sockets */
						if (cur != NULL) {	/* if accepted */
							...
							cur->k.val =
							    cur->k.restore;
							log_debug
							    ("accepted socket karma restore =>%d",
							     cur->k.
							     restore);
				...
				cur =
				    find_client(io__data->evp[i].data.fd);
...
void io_init(void)
{
	...
	if (io__data == NULL) {
		...
		if (io__data->epollfd != -1) {
			log_debug("epoll fd created: %d (size=%d)",
				  io__data->epollfd, wpj->cfg->epoll_size);
			io__data->evp =
			    pmalloco(wpj->p,
				     (wpj->cfg->epoll_maxevents +
				      1) * sizeof(struct epoll_event));
		} else {
			log_alert("epoll_error",
				  "Can't epoll_create(%d). Read README for more info",
				  wpj->cfg->epoll_size);
		}

		/* init working threads */
		pthread_create(&io__data->t, NULL, io_main, NULL);
		pthread_create(&io__data->wt, NULL, write_queue_main,
			       NULL);
	}
}
...
void io_stop(void)
{
	...
	for (cur = io__data->master__list; cur != NULL;) {
		log_alert("stop", "closing client sockets. data %d",
			  cur->queue_long);
		temp = cur;
		cur = cur->next;
		close_client(temp);
	...
	if (io__data->serverio) {
		...
		while (io__data->serverio->queue
		       && io__data->serverio->state == state_ACTIVE) {
			Sleep(100);
		}
	}
...
io io_new(int fd, void *cb, void *arg)
{
	...
	io_karma(new, KARMA_DEF_INIT, KARMA_DEF_MAX, KARMA_DEF_INC,
		 KARMA_DEF_DEC, KARMA_DEF_PENALTY, KARMA_DEF_RESTORE);
...
void io_write(io m, xmlnode x, char *buffer, int len)
{
	...
	if (buffer != NULL) {
		...
		if (m->type == type_NUL
		    && strncmp(buffer, "<stream:stream", 14)) {
			len++;
			new->data = pmalloco(p, len + 1);
			snprintf(new->data, len + 1, "%.*s/>", len - 2,
				 buffer);
		} else {
...
/*
  sets karma values
*/
void io_karma(io m, int val, int max, int inc, int dec, int penalty,
	      int restore)
{
	if (m == NULL)
		return;
...
void io_return_packet(xmlnode x)
{
	if (wpj->cfg->si->state == conn_AUTHD) {
		...
		log_alert("return",
			  "couldn't return packet %s, server down",
			  xmlnode2str(x));
		xmlnode_free(x);
	}
}
...
void io_cleanup(io m)
{
	...
	for (cur = m->queue; cur != NULL;) {
		...
		if ((wpj->cfg->no_message_storage == 0) &&
		    (temp->x) &&
		    (strncmp(xmlnode_get_name(temp->x), "message", 7) ==
		     0)) {
...
io io_listen(int port, char *listen_host, void *cb, void *arg)
{
	...
	if (fd < 0) {
		log_alert("create",
			  "io_select unable to create listen socket on %d [%s] - %s",
			  port, listen_host, strerror(errno));
		return NULL;
	}
	...
	if (listen(fd, 10) < 0) {
		log_alert("listen",
			  "io_select unable to listen on %d [%s] - %s",
			  port, listen_host, strerror(errno));
		wpj_shutdown = 1;
		return NULL;
	}
	...
	log_debug("io_select starting to listen on %d [%s]", port,
		  listen_host);
