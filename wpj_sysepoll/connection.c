...
void client_process_xml(io m, int state, void *arg, xmlnode x)
{
	...
	case IO_NEW:
		...
		log_debug("Connection opened Clients %d",
			  wpj->clients_count);
		...
		snprintf(buf, 199, "%d@%s/%s", ci->loc, wpj->cfg->hostname,
			 ci->res);
	...
	case IO_XML_ROOT:
		...
		h = xstream_header("jabber:client", NULL,
				   jid_full(ci->orghost));
		...
		if (j_strcmp
			...
			ci->state = state_CLOSING;
			io_write(ci->m, NULL,
				 "<stream:error>Did not specify a valid to argument</stream:error></stream:stream>",
				 -1);
			io_close(ci->m);

		} else
		    if (j_strcmp
			(xmlnode_get_attrib(x, "xmlns:stream"),
			 "http://etherx.jabber.org/streams") != 0) {
			ci->state = state_CLOSING;
			io_write(ci->m, NULL,
				 "<stream:error>Invalid Stream Namespace</stream:error></stream:stream>",
				 -1);
			io_close(ci->m);
		}
		xmlnode_free(x);
	...
	case IO_XML_NODE:
		...
		if (ci->state == state_UNKNOWN) {
			...
			if (*(xmlnode_get_name(x)) != 'i' ||
				...
				if (j_strcmp
					...
					xmlnode_put_attrib(q, "ip",
							   ci->m->ip);

					log_debug
					    ("Auth set packet received");

					xmlnode_put_attrib(xmlnode_get_tag
							   (q, "digest"),
							   "sid", ci->sid);
					ci->auth_id =
					    pstrdup(ci->m->p,
				...
				xmlnode_put_attrib(rt, "to",
						   jid_full(ci->orghost));

				log_debug
				    ("Sending auth packets to server");
		...
		if (ci->state == state_AUTHD) {
			...
			if (j_strncmp(xmlnode_get_name(x), "presence", 8)
			    == 0) {
				if (xmlnode_get_attrib(x, "to") == NULL) {
					log_debug
					    ("Got status presence: %s",
					     xmlnode2str(x));
					if (ci->real_user.presence)
						xmlnode_free(ci->real_user.
							     presence);
					ci->real_user.presence =
					    xmlnode_dup(x);
				}
			}
	...
	case IO_CLOSED:
		...
		if (ci->state == state_AUTHD) {
			...
			if (wpj->cfg->si->state == conn_AUTHD) {
				...
				xmlnode_put_attrib(h, "to",
						   jid_full(ci->host));
				if (wpj->cfg->si->m->state == state_ACTIVE)
					io_write(wpj->cfg->si->m, h, NULL,
						 0);
				else
					xmlnode_free(h);
			}
			...
			if (ci->orghost) {
				log_alert("Session_close", "%s [%s]",
					  jid_full(ci->orghost), m->ip);
			} else {
