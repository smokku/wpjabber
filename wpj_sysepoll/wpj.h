...
typedef enum { type_LISTEN, type_NORMAL, type_NUL, type_HTTP,
	    type_SERVER } io_type;
...
typedef int (*io_accept_func) (io m, struct sockaddr * serv_addr,
			       socklen_t * addrlen);
typedef int (*io_connect_func) (io m, struct sockaddr * serv_addr,
				socklen_t addrlen);
...
typedef void (*io_raw_cb) (io m, int state, void *arg, char *buffer,
			   int bufsz);
#ifdef HAVE_SSL
typedef void (*io_ssl_cb) (io m, int state, void *arg, char *buffer,
			   int bufsz);
#endif
...
typedef enum { conn_DIE, conn_CLOSED, conn_OPEN, conn_AUTHD,
	    conn_CLOSING } conn_state;
typedef enum { state_CLOSING, state_UNKNOWN, state_AUTHD,
	    state_REFRESHING_SESSION } client_state;
