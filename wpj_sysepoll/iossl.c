...
static void compute_temprary_rsa_keys()
{
	...
	if ((rsa512 = RSA_generate_key(512, RSA_F4, NULL, NULL)) == NULL) {
		log_alert("ssl_error",
			  "SSL:Failed to generate temporary 512 bit RSA private key");
	}

	if ((rsa1024 = RSA_generate_key(1024, RSA_F4, NULL, NULL)) == NULL) {
		log_alert("ssl_error",
			  "SSL:Failed to generate temporary 1024 bit RSA private key");
	}
}
...
int io_ssl_init(xmlnode x)
{
	...
	if (xmlnode_get_tag_data(x, "ciphers")) {
		if (SSL_CTX_set_cipher_list
		    (ctx, xmlnode_get_tag_data(x, "ciphers"))) {
			SSL *ssl;
			int i;
			STACK_OF(SSL_CIPHER) * sk;
			char buf[512];

			log_alert("config", "using ciphers: >%s<",
				  xmlnode_get_tag_data(x, "ciphers"));

			ssl = SSL_new(ctx);
			if (ssl == NULL) {
				log_alert("config",
					  "can't create ssl from ctx");
				SSL_CTX_free(ctx);
				return 1;
			}
			...
			log_alert("config", "error using ciphers: >%s<",
				  xmlnode_get_tag_data(x, "ciphers"));
	}
...
int _io_ssl_accept(io m, struct sockaddr *serv_addr, socklen_t * addrlen)
{
	...
	if (sret <= 0) {
		...
		if ((SSL_get_error(ssl, sret) == SSL_ERROR_WANT_READ) ||
			...
			m->ssl = ssl;
			pool_cleanup(m->p, _io_ssl_cleanup,
				     (void *) m->ssl);
