...
void js_mapi_register(jsmi si, event e, mcall c, void *arg)
{
	...
	log_debug("mapi_register %d %X", e, newl);
}
...
void js_mapi_session(event e, session s, mcall c, void *arg)
{
	...
	log_debug("mapi_register_session %d %X", e, newl);
}
...
int js_mapi_call(jsmi si, event e, jpacket packet, udata user, session s)
{
	...
	for (; l != NULL; l = l->next) {
		...
		if (packet != NULL
		    && (packet->type & l->mask) == packet->type)
			continue;
	...
	log_debug("mapi_call returning unhandled");
