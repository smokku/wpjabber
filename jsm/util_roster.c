...
js_roster add_roster_cache_item(udata u,jid id,int subscription){
  ...
  for (cur = (js_roster)u->roster_cache; cur; cur = cur->next){
	...
	case TYPE_USER:
	  if (!id->user) continue;
	  if (strcasecmp(cur->user,id->user)!=0) continue;
	  if (strcmp(cur->server,id->server) != 0) continue;
	  break;
	case TYPE_SERVER_RESOURCE:
	  ...
	  if (strcmp(cur->server,id->server) != 0) continue;
	  /* resource */
	  if (strcmp(cur->user,id->resource)!=0) continue;
	  break;
	case TYPE_SERVER:
	  if (id->user) continue;
	  if (id->resource) continue;
	...
	if (cur->subscription >= 100) cur->subscription -= 100;
	break;
  }
  ...
  if (cur->type == TYPE_SERVER){
	...
	log_debug("Item type %d user/res %s server %s subs %d",
			  cur->type,cur->user,cur->server,cur->subscription);
  }
...
void add_roster_item_group(udata u, js_roster cur, js_roster_group old_gr, char *name){
  ...
  for (cur_gr = old_gr;cur_gr; cur_gr = cur_gr->next){
	if (strcmp(cur_gr->name,name)==0) break;
  }
...
void ReBuildRosterCache(udata u, xmlnode roster){
  ...
  /* mark for delete */
  if (u->roster_cache){
	for (ros = (js_roster)u->roster_cache; ros; ros = ros->next)
	  ros->subscription += 100;
  }

  /* parse roster */
  p = xmlnode_pool(roster);
  for(cur = xmlnode_get_firstchild(roster); cur; cur = xmlnode_get_nextsibling(cur)){
	id = jid_new(p,xmlnode_get_attrib(cur,"jid"));
	if(id == NULL) continue;

#ifdef FORWP
	/* do not cache GG */
	if (j_strcmp(id->server,"gg.jabber.wp.pl")==0) continue;
#endif
	...
	for(gr = xmlnode_get_firstchild(cur); gr; gr = xmlnode_get_nextsibling(gr)){

	  if (xmlnode_get_type(gr) != NTYPE_TAG) continue;
	  if (j_strcmp(xmlnode_get_name(gr),"group")!=0) continue;

	  /* add group */
	  add_roster_item_group(u, ros, old_gr, xmlnode_get_data(gr));
	}
  }
  ...
  if (u->roster_cache){
	while (((js_roster)u->roster_cache)&&
		   (((js_roster)u->roster_cache)->subscription >= 100)){
	  log_debug("remove first");
	  u->roster_cache = ((js_roster)u->roster_cache)->next;
	}

	if (u->roster_cache){
	  prev = (js_roster)u->roster_cache;
	  for (ros = ((js_roster)u->roster_cache)->next; ros; ros = ros->next)
...
int load_cache_roster(udata u){
  ...
  if (u->roster_cache != NULL) return 1;
...
int js_istrusted(udata u,jid id){
  ...
  if (jid_cmpx(u->id,id, JID_USER|JID_SERVER)==0) return 1;

  if (load_cache_roster(u) == 0) return 0;
  ...
  for (cur = (js_roster)u->roster_cache; cur; cur = cur->next){
	...
	if (cur->subscription == SUBS_FROM || cur->subscription == SUBS_BOTH)
	  return 1;
	else
	  return 0;
  }
...
void js_trust_populate_presence(mapi m, jid notify){
  ...
  if (load_cache_roster(m->user) == 0) return;
  ...
  for (cur = (js_roster)m->user->roster_cache; cur; cur = cur->next){
	...
	switch (cur->type){
	case TYPE_USER:
	  snprintf(buf,500,"%s@%s",cur->user,cur->server);
	  id = jid_new(m->packet->p,buf);
	  break;
	case TYPE_SERVER_RESOURCE:
	  snprintf(buf,500,"%s/%s",cur->server,cur->user);
	  id = jid_new(m->packet->p,buf);
	  break;
	case TYPE_SERVER:
	...
	if(cur->subscription == SUBS_TO || cur->subscription == SUBS_BOTH){
	  xmlnode pnew;
...
int js_roster_item_group(udata u,jid id,char *group){
  ...
  for (cur = (js_roster)u->roster_cache; cur; cur = cur->next){
	...
	if (cur->gr == NULL){
	  ...
	  for (cur_gr = cur->gr;cur_gr; cur_gr = cur_gr->next){
		if (strcmp(cur_gr->name,group)==0) return 1;
	  }
	}
