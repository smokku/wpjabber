...
result jsm_stats(void *arg)
{
	...
	if (lt.tm_mday != si->stats->last_max_day) {
		...
		si->stats->session_max_yesterday =
		    si->stats->session_max_today;
...
int _jsm_shutdown(void *arg, const void *key, void *data)
{
	...
	log_debug("JSM SHUTDOWN: deleting users for host %s",
		  (char *) key);
...
void jsm_configure(jsmi si)
{
	...
	si->users =
	    wpxhash_new(j_atoi
			(xmlnode_get_tag_data(si->config, "maxusers"),
			 USERS_PRIME));

	si->stats = pmalloco(si->p, sizeof(_jsmi_stats));
	si->stats->started = time(NULL);
	si->stats->stats_file =
	    xmlnode_get_tag_data(si->config, "stats_file");
	...
	n = j_atoi(xmlnode_get_tag_data(cur, "deliver"),
		   DEFAULT_DELIVER_THREADS);
	si->mtq_deliver = fast_mtq_init(n, js_deliver_thread_hander, si);

	log_alert("config", "Deliver threads: %d", n);

	n = j_atoi(xmlnode_get_tag_data(cur, "authreg"),
		   DEFAULT_AUTHREG_THREADS);
	si->mtq_authreg = fast_mtq_init(n, js_authreg_thread_hander, si);

	log_alert("config", "Authreg threads: %d", n);

	n = j_atoi(xmlnode_get_tag_data(cur, "session"),
		   DEFAULT_SESSION_THREADS);
	...
	for (i = 0; i < n; i++) {
		...
		si->mtq_session[i]->mtq =
		    fast_mtq_init(1, js_session_thread_hander, si);
	...
	for (cur =
	     xmlnode_get_firstchild(xmlnode_get_tag(si->config, "admin"));
		...
		if (si->gtrust == NULL)
			...
			jid_append(si->gtrust,
				   jid_new(si->p, xmlnode_get_data(cur)));
	}
}
...
result js_users_gc_start(void *arg)
{
	...
	register_beat(j_atoi
		      (xmlnode_get_tag_data(si->config, "usergc"), 60),
		      js_users_gc, (void *) si);
...
void jsm(instance i, xmlnode x)
{
	...
	si->config =
	    xdb_get(si->xc, jid_new(xmlnode_pool(x), "config@-internal"),
		    "jabber:config:jsm");
	...
	for (cur = xmlnode_get_firstattrib(x); cur != NULL;
		...
		if ((module =
		     (modcall) xmlnode_get_firstchild(cur)) == NULL)
			continue;
