...
mreturn mod_roster_auto_in_s10n(mapi m, void *arg){
  ...
  //if from me
  if (jid_cmpx(m->s->uid,m->packet->from,JID_USER|JID_SERVER) == 0) return M_PASS;
  ...
  case JPACKET__SUBSCRIBE:
    log_debug("SUBSCRIBE");
    reply = jutil_presnew(JPACKET__SUBSCRIBED, jid_full(m->packet->from),NULL);
    js_session_from(m->s, jpacket_new(reply));
    reply = jutil_presnew(JPACKET__SUBSCRIBE, jid_full(m->packet->from),NULL);
    js_session_from(m->s, jpacket_new(reply));
    break;
  ...
  case JPACKET__UNSUBSCRIBE:
    ...
    xmlnode_put_attrib(x,"jid",jid_full(jid_user(m->packet->from)));
