...
void _mod_presence_broadcast_trusted(session s, jid notify, xmlnode x){
    ...
    for(cur = notify; cur != NULL; cur = cur->next){
	  if(!js_istrusted(s->u,cur)) continue; /* perform insersection search, must be in both */
	  s->c_out++;
	  pres = xmlnode_dup(x);
	  xmlnode_put_attrib(pres, "to",jid_full(cur));
...
mreturn mod_presence_out(mapi m, void *arg){
	...
	if(oldpri >= -128){
		    js_session_from(m->s, jpacket_new(jutil_presnew(JPACKET__UNAVAILABLE,NULL,
...
mreturn mod_presence_deliver(mapi m, void *arg){
    ...
    if(m->user != NULL && m->packet->to->resource == NULL && js_session_primary_sem(m->user) != NULL){
