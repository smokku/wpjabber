...
inline int filter_subscription(char *subs)
{
	...
	if (strcmp(subs, "trusted") == 0)
		return SUBSCRIPTION_TRUSTED;
	if (strcmp(subs, "from") == 0)
		return SUBSCRIPTION_TRUSTED;
	if (strcmp(subs, "both") == 0)
		return SUBSCRIPTION_TRUSTED;
	return SUBSCRIPTION_NOTTRUSTED;
	if (strcmp(subs, "none") == 0)
...
inline int filter_type(char *type)
{
	if (type == NULL)
		return RULE_GLOBAL;
	if (type[0] == 'j')
		return RULE_JID_MATCH;
	if (type[0] == 's')
		return RULE_SUBSCRIPTION_MATCH;
	if ((type[0] == 'g') && (type[1] == 'r'))
		return RULE_GROUP_MATCH;
	return RULE_GLOBAL;
}
...
{
	cur->filter = 0;
	if (xmlnode_get_tag(x, "message"))
		cur->filter |= BLOCK_MESSAGE;
	if (xmlnode_get_tag(x, "iq"))
		cur->filter |= BLOCK_IQ;
	if (xmlnode_get_tag(x, "presence-in"))
		cur->filter |= BLOCK_PRESENCE_IN;
	if (xmlnode_get_tag(x, "presence-out"))
		cur->filter |= BLOCK_PRESENCE_OUT;
}
		...
		case ACTION_RULE_ALLOW:
			...
			if (rule->type == RULE_JID_MATCH) {
				...
				if (j == NULL)
					continue;
				rule->jid =
				    pstrdup(filters->p, jid_full(j));
				if (j->user) {
					unsigned char *str;
					for (str = rule->jid; *str != '@';
					     str++) {
						*str = tolower(*str);
					}
				}
			} else if (rule->type == RULE_SUBSCRIPTION_MATCH) {
				rule->subscription =
				    filter_subscription(xmlnode_get_attrib
							(cur, "value"));
			} else if (rule->type == RULE_GROUP_MATCH) {
				rule->jid =
				    pstrdup(filters->p,
					    xmlnode_get_attrib(cur,
							       "value"));
...
int CheckUserFilters(xmlnode x)
{
	...
	for (cur = x; cur != NULL; cur = xmlnode_get_nextsibling(cur)) {
		...
		/* check value */
		if (type != RULE_GLOBAL) {
			if (xmlnode_get_attrib(cur, "value") == NULL)
				continue;
			if (type == RULE_JID_MATCH) {
				if (!jid_full
				    (jid_new
				     (xmlnode_pool(x),
				      xmlnode_get_attrib(cur, "value"))))
					continue;
			}
		}
		...
		log_debug("CHECK: found rule action: %d type %d", action,
			  type);
...
/* -------------------------------------------------------------------------
   ...
   *************************************************************************
   ...
   ------------------------------------------------------------------------- */
/* if denied */
inline mreturn filter_denied(filter_rule filter, user_filters filters,
			     mapi m, int denied_type)
{
	...
	if (m->packet->type == JPACKET_MESSAGE) {
	      message_packet:
		{
			...
			if (id) {
				for (cur =
				     xmlnode_get_firstchild(m->packet->x);
				     cur;
...
inline mreturn filter_do_action(filter_rule cur, user_filters filters,
				mapi m, int denied_type)
{
	switch (cur->action) {
	case ACTION_RULE_DENIED:
...
inline int load_user_filters(user_filters filters, mapi m)
{
	...
	if (filters->p == NULL) {
		if (filters->filters == NULL) {
			...
			xdb_filters =
			    xdb_get(m->si->xc, m->user->id, NS_PRIVACY);
			if (xdb_filters == NULL) {
				filters->filters = NO_FILTERS;
				return 0;
			}

			log_debug("found users filters: %s",
				  xmlnode2str(xdb_filters));
...
mreturn mod_privacy_in(mapi m, void *arg)
{
	...
	if (m->packet->from) {
		if (m->packet->from->user == NULL)
			if (strcmp(m->packet->from->server, m->si->host) ==
			    0)
				return M_PASS;
	...
	for (cur = filters->filters; cur != NULL; cur = cur->next) {
		...
		if (cur->filter) {
			...
			case JPACKET_IQ:
				...
				if (!(cur->filter & BLOCK_IQ))
					continue;
				break;
			case JPACKET_S10N:
				//        case JPACKET_PRESENCE:
				/* XXX if filter is not for PRESENCE packet */
				if (!(cur->filter & BLOCK_PRESENCE_IN))
					continue;
				break;
			default:
		...
		case RULE_GLOBAL:
			...
			return filter_do_action(cur, filters, m,
						DENIED_GLOBAL);
			break;
		case RULE_SUBSCRIPTION_MATCH:
			if (m->packet->from) {
				...
				is_trusted =
				    js_trust(m->user, m->packet->from);


				log_debug
				    ("COMPARE SUBSCRIPTION from %d, cur %d",
				     is_trusted, cur->subscription);

				if (is_trusted == cur->subscription) {
					log_debug("SUBSCRIPTION RULE");
					return filter_do_action(cur,
								filters, m,
								DENIED_GLOBAL);
				}
			}
		...
		case RULE_GROUP_MATCH:
			if (m->packet->from) {
				if (js_roster_item_group
				    (m->user, m->packet->from, cur->jid)) {
		...
		case RULE_JID_MATCH:
			...
			if (m->packet->from) {
				if (compare_jid
				    (jid_full(m->packet->from), cur->jid,
				     m->packet->from->user != NULL))
					continue;
...
mreturn mod_privacy_iq_out(mapi m, void *arg)
{
	...
	case JPACKET__SET:
		{
			...
			if (list == NULL) {
				...
				xmlnode_put_attrib(m->packet->x, "type",
						   "error");
				xmlnode_put_attrib(xmlnode_insert_tag
						   (m->packet->x, "error"),
						   "code", "406");
				xmlnode_insert_cdata(xmlnode_get_tag
						     (m->packet->x,
						      "error"), "No list",
						     -1);
			...
			case 1:
				...
				/* save */
				xdb_set(m->si->xc, m->user->id, NS_PRIVACY,
					NULL);
				jutil_iqresult(m->packet->x);
				break;
	...
	case JPACKET__GET:
		...
		xmlnode_insert_node(m->packet->iq,
				    xmlnode_get_firstchild(xdb_filters));
...
mreturn mod_privacy_session(mapi m, void *arg)
{
	...
	js_mapi_session(es_OUT, m->s, mod_privacy_iq_out,
			(void *) filters);
	/* handle incoming packets */
	js_mapi_session(es_IN, m->s, mod_privacy_in, (void *) filters);
	/* handle session outgoing packets */
	js_mapi_session(es_POSTOUT, m->s, mod_privacy_postout,
			(void *) filters);
