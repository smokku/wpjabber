...
void mod_roster_push(udata user, xmlnode item){ /* push the item to all session */
    session cur;
...
mreturn mod_roster_out_s10n(mapi m,roster_cfg roster_config){
	...
	if(!to){
		  ...
		  for(cur = xmlnode_get_firstchild(roster);cur != NULL;cur = xmlnode_get_nextsibling(cur)){
			...
			if (xmlnode_get_attrib(cur,"jid") == NULL)  continue;
#ifdef FORWP
...
mreturn mod_roster_out(mapi m, void *arg){
    ...
    if(m->packet->type == JPACKET_IQ) return mod_roster_out_iq(m,roster_config);
    if(m->packet->type == JPACKET_S10N) return mod_roster_out_s10n(m,roster_config);
...
mreturn mod_roster_s10n(mapi m, void *arg){
    ...
    log_debug("s10n %s request from %s with existing item %s",xmlnode_get_attrib(m->packet->x,"type"),jid_full(m->packet->from),xmlnode2str(item));
...
JSM_FUNC void mod_roster(jsmi si){
  ...
  if(temp){
	roster_config->max_roster_size = j_atoi(xmlnode_get_data(
															  xmlnode_get_tag(temp,"maxsize")
															  ),100);
	roster_config->error_msg = pstrdup(si->p, xmlnode_get_tag_data(temp, "message"));
	roster_config->error_from = pstrdup(si->p, xmlnode_get_tag_data(temp, "from"));
  }
  else{
	roster_config->max_roster_size = 100;
	roster_config->error_msg = pstrdup(si->p, "Your roster is too big !");
