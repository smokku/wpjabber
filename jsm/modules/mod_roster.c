...
typedef struct roster_cfg_struct {
	int max_roster_size;
	char *error_msg;
	char *error_from;
} _roster_cfg, *roster_cfg;

/* inform user in error message from admin */
void error_message(mapi m, roster_cfg roster_config)
{
	xmlnode temp = jutil_msgnew(NULL,
				    jid_full(m->s->id),
				    "Error",
				    roster_config->error_msg);
	xmlnode_put_attrib(temp, "from", roster_config->error_from);
	js_session_to(m->s, jpacket_new(temp));
}
...
xmlnode mod_roster_get(udata u)
{
	...
	log_debug("getting %s's roster", u->user);

	/* get the existing roster */
	ret = xdb_get(u->si->xc, u->id, NS_ROSTER);
	if (ret == NULL) {	/* there isn't one, sucky, create a container node and let xdb manage it */
		log_debug("creating");
...
xmlnode mod_roster_get_item(xmlnode roster, jid id, int *newflag)
{
	...
	log_debug("getting item %s", jid_full(id));

	ret = jid_nodescan(id, roster);

	if (ret == NULL) {	/* there isn't one, brew one up */
		log_debug("creating");
...
{				/* push the item to all session */
	...
	log_debug("pushing %s", xmlnode2str(item));
	...
	for (cur = user->sessions; cur != NULL; cur = cur->next)
		if (cur->roster)
			js_session_to(cur,
				      jpacket_new(xmlnode_dup(packet)));
...
void mod_roster_pforce(udata u, jid to, int uflag)
{
	...
	log_debug("brute forcing presence updates");

	/* loop through all the sessions */
	SEM_LOCK(u->sem);
	for (s = u->sessions; s != NULL; s = s->next) {
		if (uflag)
			x = jutil_presnew(JPACKET__UNAVAILABLE, NULL,
					  NULL);
		else
...
mreturn mod_roster_out_s10n(mapi m, roster_cfg roster_config)
{
	xmlnode roster, item, cur;
	int newflag, to, from, count;

	if (m->packet->to == NULL)
		return M_PASS;
	//    if(jid_cmpx(jid_user(m->s->id),m->packet->to,JID_USER|JID_SERVER) == 0) return M_PASS; /* vanity complex */
	if (jid_cmpx(m->s->uid, m->packet->to, JID_USER | JID_SERVER) == 0)
		return M_PASS;	/* vanity complex */

	log_debug("handling outgoing s10n");
	...
	if (j_strcmp(xmlnode_get_attrib(item, "subscription"), "to") == 0)
		to = 1;
	if (j_strcmp(xmlnode_get_attrib(item, "subscription"), "from") ==
	    0)
		from = 1;
	if (j_strcmp(xmlnode_get_attrib(item, "subscription"), "both") ==
	    0)
		to = from = 1;
	...
	case JPACKET__SUBSCRIBE:
		if (!to) {
			count = 0;
			for (cur = xmlnode_get_firstchild(roster);
			     cur != NULL;
			...
			/* check */
			if (count >= roster_config->max_roster_size) {
				/* free temp roster data */
				xmlnode_free(roster);
				xmlnode_free(m->packet->x);

				/* inform user */
				error_message(m, roster_config);
	...
	case JPACKET__SUBSCRIBED:
		...
		//              js_trust_add(m->user,m->packet->to,1); /* make them trusted now */
	...
	case JPACKET__UNSUBSCRIBED:
		if (from) {
			...
			//                      js_trust_remove(m->user,m->packet->to);
...
mreturn mod_roster_out_iq(mapi m, roster_cfg roster_config)
{
	...
	case JPACKET__GET:
		...
		for (cur = xmlnode_get_firstchild(m->packet->iq);
		...
		for (cur = xmlnode_get_firstchild(roster); cur != NULL;
			...
			if (xmlnode_get_attrib(cur, "subscribe") != NULL) {
				...
				xmlnode_put_attrib(pres, "type",
						   "subscribe");
				xmlnode_put_attrib(pres, "from",
						   xmlnode_get_attrib(cur,
								      "jid"));
				if (strlen
				    (xmlnode_get_attrib(cur, "subscribe"))
				    > 0)
		...
		break;
	case JPACKET__SET:
		log_debug("handling set request");

		/* count our roster items */
		count = 0;
		for (cur = xmlnode_get_firstchild(roster); cur != NULL;
		     cur = xmlnode_get_nextsibling(cur)) {
			if (xmlnode_get_type(cur) != NTYPE_TAG)
			...
			if (strstr
				...
				continue;
#endif
			//                      if (xmlnode_get_attrib(cur,"subscribe") != NULL) continue;
		...
		for (cur = xmlnode_get_firstchild(m->packet->iq);
			...
			if (j_strcmp
				...
				if (j_strcmp
				    (xmlnode_get_attrib
				     (item, "subscription"), "both") == 0
				    ||
				    j_strcmp(xmlnode_get_attrib
					     (item, "subscription"),
					     "to") == 0
				    ||
				    j_strcmp(xmlnode_get_attrib
			...
			xmlnode_put_attrib(cur, "subscription",
		...
		/* save the changes */
		log_debug("SROSTER: %s", xmlnode2str(roster));
		/* XXX what do we do if the set fails?  hrmf... */
		js_roster_save(m->user, roster);
		//        xdb_set(m->si->xc, m->user->id, NS_ROSTER, roster);
...
mreturn mod_roster_out(mapi m, void *arg)
{
	...
	if (m->packet->type == JPACKET_IQ)
		return mod_roster_out_iq(m, roster_config);
	if (m->packet->type == JPACKET_S10N)
		return mod_roster_out_s10n(m, roster_config);
...
mreturn mod_roster_s10n(mapi m, void *arg)
{
	...
	log_debug("s10n %s request from %s with existing item %s",
		  xmlnode_get_attrib(m->packet->x, "type"),
		  jid_full(m->packet->from), xmlnode2str(item));
	...
	case JPACKET__UNSUBSCRIBE:
		...
		reply =
		    jutil_presnew(JPACKET__UNSUBSCRIBED,
				  jid_full(m->packet->from), "Autoreply");
		jid_set(m->packet->to, NULL, JID_RESOURCE);
	...
	/* XXX what do we do if the set fails?  hrmf... */
	xdb_set(m->si->xc, m->user->id, NS_ROSTER, roster);
	//      XXX wh change user js_trust js_roster_save(m->user,roster);
...
JSM_FUNC void mod_roster(jsmi si)
{
	xmlnode temp;
	roster_cfg roster_config;

	roster_config = pmalloco(si->p, sizeof(_roster_cfg));
	...
	if (temp) {
		...
		roster_config->error_from =
		    pstrdup(si->p, xmlnode_get_tag_data(temp, "from"));
	} else {
