...
result mod_packet_stats_beat(void *arg){
  ...
  for (sp = lp->stats; sp; sp = sp->next){
    ...
    msg    = sp->messages;	sp->messages	  = 0;
    pres   = sp->presences;	sp->presences	  = 0;
    query  = sp->queries;	sp->queries	  = 0;
#ifdef FORWP
    files  = sp->files;		sp->files	  = 0;
    voip   = sp->voip;		sp->voip	  = 0;
    vm	   = sp->voice_message; sp->voice_message = 0;
#endif
    ...
    if (f){
#ifdef FORWP
      fprintf(f, "%s m:\t%u\tp:\t%u\tq:\t%u\tf:\t%u\tvo:\t%u\tvm:\t%u\r\n",
	      stamp,
	      msg,
	      pres,
	      query,
...
mreturn mod_packet_stats_out(mapi m, void *arg){
  ...
  if (!m->packet->to)
    ...
    for (sp = lp->stats; sp; sp = sp->next){
      if (!sp->domain) break;
      if (j_strcmp(sp->domain,m->packet->to->server)== 0) break;
    }
  }
  ...
  if(m->packet->type == JPACKET_MESSAGE){
...
#ifdef FORWP
    for (x = xmlnode_get_tag(m->packet->x,"x"); x; x = xmlnode_get_nextsibling(x)){

      if (xmlnode_get_type(x) != NTYPE_TAG) continue;
      if (j_strcmp(xmlnode_get_name(x),"x")) continue;

      if (j_strcmp(xmlnode_get_attrib(x,"xmlns"), "wpmsg:x:voice") == 0){
	...
	return M_PASS;
      ...
      if (j_strcmp(xmlnode_get_attrib(x,"type"), "req") == 0){
	char *service;
	service = xmlnode_get_tag_data(x,"service");
...
JSM_FUNC void mod_packet_stats(jsmi si){
  ...
  if(temp){
    ...
    lp->time = j_atoi(xmlnode_get_tag_data(temp,"interval"),DEFAULT_STATS_INTERVAL);
    ...
    for (cur = xmlnode_get_tag(temp,"file"); cur;
	 cur = xmlnode_get_nextsibling(cur)){
      ...
      if (j_strcmp(xmlnode_get_name(cur),"file")) continue;


      file = xmlnode_get_data(cur);
      domain = xmlnode_get_attrib(cur,"domain");

      log_debug("config_packet_stats %s  %s %s",xmlnode2str(cur),
		file, domain ? domain : "other");
	...
	if (domain){
	  log_debug("mod_packet_stats %s %s", file, domain);

	  sp->domain = pstrdup(si->p, domain);

	  //add at the begining or on the second place
	  if (j_strcmp(sp->domain, si->host) == 0){
	    sp->next = lp->stats;
	    lp->stats = sp;
	  }
	  else{
	  ...
	  if (nodomain){
	    log_alert("mod_packet_stats","error: two global logs");
	    return;
	  }
	  nodomain = 1;

	  log_debug("mod_packet_stats %s other", file);
	  ...
	  if (!lp->stats)
	    ...
	    log_stats tmp;
	    for (tmp = lp->stats; tmp->next ; tmp = tmp->next);
	    tmp->next = sp;
	  }
	}
	count++;
	...
	log_debug("config_packet_stats %s %s",
		  sp->file,
		  (sp->domain ? sp->domain : "other"));
      }

      js_mapi_register(si, e_SESSION, mod_packet_stats_session, (void *)lp);
      register_beat(lp->time, mod_packet_stats_beat, (void *)lp);
    }
  }
}
