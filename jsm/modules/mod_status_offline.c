...
mreturn mod_status_offline_out(mapi m, void *arg){
  ...
  if ( (m->packet->to != NULL ||
		(jpacket_subtype(m->packet) != JPACKET__UNAVAILABLE)))	return M_PASS;
  ...
  if (status){
	...
	p = pool_heap(strlen(status) + strlen(m->user->user) + sizeof(_status_bucket) + 10);
	sbnew = pmalloco(p, sizeof(_status_bucket));
	sbnew->p = p;
	sbnew->status = pstrdup(p, status);
	sbnew->last = time(NULL);

	log_debug("Set new status %s for user %s",status, m->user->user);

	wpxhash_put(so->hash, pstrdup(p, m->user->user), (void *)sbnew);
	...
	sb = (status_bucket) wpxhash_get(so->hash, m->user->user);
	if (sb){
	  log_debug("Remove status for user %s",m->user->user);
...
mreturn mod_status_offline_offline(mapi m, void *arg){
  ...
  if (sb){
	...
	x = jutil_presnew(JPACKET__UNAVAILABLE, jid_full(m->packet->from), sb->status);

    delay = xmlnode_insert_tag(x,"x");
    xmlnode_put_attrib(delay,"xmlns",NS_DELAY);

    localtime_r(&(sb->last),&new_time);
	snprintf(stamp, 18, "%d%02d%02dT%02d:%02d:%02d", 1900+new_time.tm_year,
...
result mod_status_check(void *arg){
  ...
  if ((so->count > CLEAR_STATUS_HASH_SIZE)||
	...
	log_alert("offline_status","Hash clear. Count %d",so->count);
  ...
  if (so->last_hash_realloc + HASH_REALLOC_TIMEOUT < so->now){
	...
	SEM_LOCK(so->sem);
    wpxhash_walk(so->hash, _realloc_offline_status, (void *)so);
	SEM_UNLOCK(so->sem);
  }
...
void mod_status_offline(jsmi si){
  ...
  if (xmlnode_get_tag(si->config, "mod_status_offline")){
	...
	so->hash = wpxhash_new_pool(DEFAULT_STATUS_HASH_SIZE, si->p);
	SEM_INIT(so->sem);
	so->count = 0;
	so->last_hash_clear	= time(NULL);
	so->last_hash_realloc = time(NULL);

	js_mapi_register(si,e_SHUTDOWN, mod_status_offline_shutdown,(void*)so);
	js_mapi_register(si,e_SESSION,mod_status_offline_session,(void*)so);
	js_mapi_register(si,e_OFFLINE,mod_status_offline_offline,(void*)so);
    register_beat(BEAT_OFFLINE_STATUS,mod_status_check,(void *)so);
  }
}
