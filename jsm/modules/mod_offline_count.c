...
mreturn mod_offline_count_message(mapi m, void *arg)
{
	...
	if (off->ns) {
		...
		if (cur2) {
			...
			count =
			    j_atoi(xmlnode_get_attrib(cur2, "count"), 0);
			if (count > 0) {
				if (j_strcmp
				    (jid_from,
				     xmlnode_get_attrib(cur2,
							"lastfrom")) == 0)
					change = 0;
			}
			...
			if (change) {
				...
				xmlnode_put_attrib(cur2, "lastfrom",
						   jid_from);
				xdb_set(m->si->xc, m->user->id, off->ns,
					cur2);
			...
			if (off->host) {
				send_event(m, off, jid_full(m->user->id),
					   1, jid_from);
			}
...
mreturn mod_offline_count_server(mapi m, void *arg)
{
	...
	if (j_strcmp(m->packet->from->server, off->host) != 0)
		return M_PASS;

	/* if not our command */
	if (!NSCHECK(m->packet->iq, "jabber:iq:offlinecount"))
		return M_PASS;
