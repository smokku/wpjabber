...
mreturn mod_xml_set(mapi m, void *arg)
{
	...
	if (to != NULL) {
		...
		log_debug("handling user request %s",
	...
	case JPACKET__GET:
		log_debug("handling get request for %s", ns);

		xmlnode_put_attrib(m->packet->x, "type", "result");

		/* insert the chunk into the parent, that being either
		   the iq:private container or the iq itself */
		if ((storedx = xdb_get(m->si->xc, to, ns)) != NULL) {
			if (private)	/* hack, ick! */
				xmlnode_hide_attrib(storedx,
						    "j_private_flag");
			xmlnode_insert_tag_node(xmlnode_get_parent(inx),
						storedx);
			xmlnode_hide(inx);
	...
	case JPACKET__SET:
		log_debug("handling set request for %s with data %s", ns,
			  xmlnode2str(inx));
		...
		if (to->resource != NULL) {
			if ((storedx = xdb_get(m->si->xc, to, NS_BROWSE)) == NULL) {	/* send an iq set w/ a generic browse item for this resource */
				jp = jpacket_new(jutil_iqnew
						 (JPACKET__SET,
						  NS_BROWSE));
				storedx =
				    xmlnode_insert_tag(jp->iq, "item");
				xmlnode_put_attrib(storedx, "jid",
						   jid_full(to));
...
mreturn mod_xml_get(mapi m, void *arg)
{
	...
	log_debug("handling %s request for user %s", ns,
	...
	if (xmlnode_get_attrib(xns, "j_private_flag") != NULL) {	/* uhoh, set from a private namespace */
		js_bounce(m->si, m->packet->x, TERROR_NOTALLOWED);
		return M_HANDLED;
	}
