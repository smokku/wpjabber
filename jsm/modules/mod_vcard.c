...
mreturn mod_vcard_jud(mapi m)
{
	...
	if (vcard != NULL) {
		log_debug("sending registration for %s",
		...
		xmlnode_insert_cdata(xmlnode_insert_tag(regq, "key"), key,
				     -1);

		xmlnode_insert_cdata(xmlnode_insert_tag(regq, "name"),
				     xmlnode_get_tag_data(vcard, "FN"),
				     -1);
		xmlnode_insert_cdata(xmlnode_insert_tag(regq, "first"),
				     xmlnode_get_tag_data(vcard,
							  "N/GIVEN"), -1);
		xmlnode_insert_cdata(xmlnode_insert_tag(regq, "last"),
				     xmlnode_get_tag_data(vcard,
							  "N/FAMILY"), -1);
		xmlnode_insert_cdata(xmlnode_insert_tag(regq, "nick"),
				     xmlnode_get_tag_data(vcard,
							  "NICKNAME"), -1);
		xmlnode_insert_cdata(xmlnode_insert_tag(regq, "email"),
				     xmlnode_get_tag_data(vcard, "EMAIL"),
				     -1);
...
mreturn mod_vcard_set(mapi m, void *arg)
{
	...
	if (m->packet->type != JPACKET_IQ)
		return M_IGNORE;
	if (m->packet->to != NULL || !NSCHECK(m->packet->iq, NS_VCARD))
		return M_PASS;
	...
	case JPACKET__GET:
		...
		break;
	case JPACKET__SET:
		log_debug("handling set request %s",
			  xmlnode2str(m->packet->iq));

		/* save and send response to the user */
		if (xdb_set
		    (m->si->xc, m->user->id, NS_VCARD, m->packet->iq)) {
			/* failed */
		...
		for (cur =
		     xmlnode_get_firstchild(js_config(m->si, "browse"));
		     cur != NULL; cur = xmlnode_get_nextsibling(cur)) {
			if (j_strcmp
			    (xmlnode_get_attrib(cur, "type"), "jud") != 0)
				continue;

			judreg = jutil_iqnew(JPACKET__GET, NS_REGISTER);
			xmlnode_put_attrib(judreg, "to",
...
mreturn mod_vcard_reply(mapi m, void *arg)
{
	...
	if (m->packet->type != JPACKET_IQ)
		return M_IGNORE;
	if (j_strcmp
	    (xmlnode_get_attrib(m->packet->x, "id"), "mod_vcard_jud") == 0)
...
mreturn mod_vcard_server(mapi m, void *arg)
{
	...
	log_debug("handling server vcard query");
