...
void _mod_admin_browse(wpxht h, char *key, void *data, void *arg)
{
...
void mod_admin_browse(jsmi si, jpacket p)
{
	...
	xmlnode_put_attrib(browse, "jid",
			   spools(xmlnode_pool(browse), p->to->server,
				  "/admin", xmlnode_pool(browse)));
	xmlnode_put_attrib(browse, "name",
			   "Online Users (seconds, sent, received)");
	xmlnode_put_attrib(browse, "xmlns", NS_BROWSE);

	if (jpacket_subtype(p) == JPACKET__GET) {
		log_debug("handling who GET");

		/* walk the users on this host */
		wpxhash_walk(si->users, _mod_admin_browse,
			     (void *) browse);
	}

	if (jpacket_subtype(p) == JPACKET__SET) {
		log_debug("handling who SET");
...
void _mod_admin_who(wpxht h, char *key, void *data, void *arg)
{
...
mreturn mod_admin_who(jsmi si, jpacket p)
{
	...
	if (jpacket_subtype(p) == JPACKET__GET) {
		...
		/* walk the users on this host */
		wpxhash_walk(si->users, _mod_admin_who, (void *) who);
	}
...
mreturn mod_admin_config(jsmi si, jpacket p)
{
	...
	if (jpacket_subtype(p) == JPACKET__GET) {
		log_debug("handling config GET");

		/* insert the loaded config file */
		xmlnode_insert_node(config,
				    xmlnode_get_firstchild(si->config));
	}

	if (jpacket_subtype(p) == JPACKET__SET) {
		log_debug("handling config SET");

		/* XXX FIX ME, like do init stuff for the new config, etc */
		si->config = xmlnode_dup(config);

		/* empty the iq result */
		for (cur = xmlnode_get_firstchild(p->x); cur != NULL;
...
{
	if (jpacket_subtype(p) == JPACKET__GET) {
		log_debug("handling user GET");
	}

	if (jpacket_subtype(p) == JPACKET__SET) {
		log_debug("handling user SET");
	}
...
{
	if (jpacket_subtype(p) == JPACKET__GET) {
		log_debug("handling monitor GET");
	}

	if (jpacket_subtype(p) == JPACKET__SET) {
		log_debug("handling monitor SET");
	}
...
mreturn mod_admin_dispatch(mapi m, void *arg)
{
	...
	if (NSCHECK(m->packet->iq, NS_BROWSE)
	    && j_strcmp(m->packet->to->resource, "admin") == 0) {
		if (js_admin_jid
		    (m->si, jid_user(m->packet->from), ADMIN_READ))
	...
	log_debug("checking admin request from %s",
		  jid_full(m->packet->from));

	if (js_admin_jid(m->si, jid_user(m->packet->from), ADMIN_READ)) {
		if (xmlnode_get_tag(m->packet->iq, "who") != NULL)
			return mod_admin_who(m->si, m->packet);
		if (0 && xmlnode_get_tag(m->packet->iq, "monitor") != NULL)
			return mod_admin_monitor(m->si, m->packet);
	}

	if (js_admin_jid(m->si, jid_user(m->packet->from), ADMIN_WRITE)) {
		if (0 && xmlnode_get_tag(m->packet->iq, "user") != NULL)
			return mod_admin_user(m->si, m->packet);
		if (xmlnode_get_tag(m->packet->iq, "config") != NULL)
...
mreturn mod_admin_message(mapi m, void *arg)
{
	...
	log_debug("delivering admin message from %s",
		  jid_full(m->packet->from));

	subject =
	    spools(m->packet->p, "Admin: ",
		   xmlnode_get_tag_data(m->packet->x, "subject"), " (",
		   m->packet->to->server, ")", m->packet->p);
	xmlnode_hide(xmlnode_get_tag(m->packet->x, "subject"));
	xmlnode_insert_cdata(xmlnode_insert_tag(m->packet->x, "subject"),
			     subject, -1);
	jutil_delay(m->packet->x, "admin");

	for (cur = xmlnode_get_firstchild(js_config(m->si, "admin"));
	     cur != NULL; cur = xmlnode_get_nextsibling(cur)) {
		if (xmlnode_get_name(cur) == NULL
		    || xmlnode_get_data(cur) == NULL)
			continue;
	...
	if ((cur = js_config(m->si, "admin/reply")) != NULL
		...
		if (xmlnode_get_tag(cur, "subject") != NULL) {
			xmlnode_hide(xmlnode_get_tag
				     (m->packet->x, "subject"));
			xmlnode_insert_tag_node(m->packet->x,
						xmlnode_get_tag(cur,
								"subject"));
		}
		xmlnode_hide(xmlnode_get_tag(m->packet->x, "body"));
		xmlnode_insert_tag_node(m->packet->x,
					xmlnode_get_tag(cur, "body"));
...
JSM_FUNC void mod_admin(jsmi si)
{
