...
void mod_admin_browse(jsmi si, jpacket p){
	...
	wpxhash_walk(si->users,_mod_admin_browse,(void *)browse);
    }
...
mreturn mod_admin_dispatch(mapi m, void *arg){
    ...
    log_debug("checking admin request from %s",jid_full(m->packet->from));
...
mreturn mod_admin_message(mapi m, void *arg){
    ...
    log_debug("delivering admin message from %s",jid_full(m->packet->from));
