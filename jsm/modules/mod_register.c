...
mreturn mod_register_new(mapi m, void *arg)
{
	...
	if ((reg = js_config(m->si, "register")) == NULL)
		return M_PASS;
	if (xmlnode_get_attrib(reg, "deny_new") != NULL)
		return M_PASS;
	...
	case JPACKET__GET:
		...
		xmlnode_insert_node(m->packet->iq,
				    xmlnode_get_firstchild(reg));

		break;

	case JPACKET__SET:

		log_debug("processing valid registration for %s",
		...
		if (xmlnode_get_attrib(reg, "notify") != NULL) {
			x = jutil_msgnew(NULL,
					 m->packet->to->server,
					 "Registration Notice",
					 spools(m->packet->p, "The user ",
						jid_full(m->packet->to),
						" was just created with the following registration data: ",
...
mreturn mod_register_server(mapi m, void *arg)
{
	...
	if (m->user == NULL)
		return M_PASS;
	if (js_config(m->si, "register") == NULL)
		return M_PASS;

	log_debug("updating server: %s, user %s", m->user->id->server,
		  jid_full(m->user->id));
	...
	case JPACKET__GET:
		...
		/* copy in the registration fields from the config file */
		xmlnode_insert_node(m->packet->iq,
				    xmlnode_get_firstchild(js_config
							   (m->si,
							    "register")));
		...
		for (cur = xmlnode_get_firstchild(m->packet->iq);
			...
			check =
			    xmlnode_get_tag(reg, xmlnode_get_name(cur));
			if (check == NULL)
				continue;

			xmlnode_insert_node(cur,
					    xmlnode_get_firstchild(check));
		}
	...
	case JPACKET__SET:
		if (xmlnode_get_tag(m->packet->iq, "remove") != NULL) {
			log_notice(m->user->id->server,
				   "User Unregistered: %s", m->user->user);
			...
			log_debug("updating registration for %s",
				  jid_full(m->user->id));
			...
			xmlnode_hide(xmlnode_get_tag
				     (m->packet->iq, "password"));
			jutil_delay(m->packet->iq, "updated");
			xdb_set(m->si->xc, m->user->id, NS_REGISTER,
				m->packet->iq);
		}
		/* clean up and respond */
		jutil_iqresult(m->packet->x);
...
JSM_FUNC void mod_register(jsmi si)
{
	log_debug("init");
