...
mreturn mod_register_new(mapi m, void *arg){
	...
	log_debug("processing valid registration for %s",jid_full(m->packet->to));
...
mreturn mod_register_server(mapi m, void *arg){
    ...
    log_debug("updating server: %s, user %s",m->user->id->server,jid_full(m->user->id));
	...
	if(xmlnode_get_tag(m->packet->iq,"remove") != NULL){
	    ...
	    log_debug("updating registration for %s",jid_full(m->user->id));
